...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[paperclaw] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`paperclaw worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".paperclaw", "dev-server-status.json");
const devServerStatusToken = mode === "dev" ? randomUUID() : null;
const devServerStatusTokenHeader = "x-paperclaw-dev-server-status-token";
...
const ignoredRelativePaths = new Set([
  ".paperclaw/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[paperclaw] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[paperclaw] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[paperclaw] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  PAPERCLAW_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.PAPERCLAW_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.PAPERCLAW_DEV_SERVER_STATUS_TOKEN = devServerStatusToken ?? "";
  env.PAPERCLAW_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  delete env.PAPERCLAW_DEV_SERVER_STATUS_TOKEN;
  env.PAPERCLAW_MIGRATION_PROMPT ??= "never";
  env.PAPERCLAW_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[paperclaw] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.PAPERCLAW_BIND = effectiveBind;
  if (bindHost) {
    env.PAPERCLAW_BIND_HOST = bindHost;
  } else {
    delete env.PAPERCLAW_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.PAPERCLAW_DEPLOYMENT_MODE;
    delete env.PAPERCLAW_DEPLOYMENT_EXPOSURE;
    delete env.PAPERCLAW_AUTH_BASE_URL_MODE;
    console.log("[paperclaw] dev mode: local_trusted (bind=loopback)");
  } else {
    env.PAPERCLAW_DEPLOYMENT_MODE = "authenticated";
    env.PAPERCLAW_DEPLOYMENT_EXPOSURE = "private";
    env.PAPERCLAW_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[paperclaw] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.PAPERCLAW_BIND;
  delete env.PAPERCLAW_BIND_HOST;
  delete env.PAPERCLAW_DEPLOYMENT_MODE;
  delete env.PAPERCLAW_DEPLOYMENT_EXPOSURE;
  delete env.PAPERCLAW_AUTH_BASE_URL_MODE;
  console.log("[paperclaw] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[paperclaw] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@kesarcloud/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[paperclaw] Command failed with code ${status.code}: pnpm --filter @kesarcloud/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[paperclaw] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.PAPERCLAW_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[paperclaw] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[paperclaw] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@kesarcloud/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[paperclaw] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@kesarcloud/server", serverScript, ...forwardedArgs],
