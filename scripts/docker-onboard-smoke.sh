...
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@kesarcloud.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-paperclaw-smoke-password}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/paperclaw-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$PAPERCLAW_PUBLIC_URL/api/health" 90 1; then
