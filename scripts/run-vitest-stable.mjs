...
function runVitest(args, label) {
  console.log(`\n[test:run] ${label}`);
  invocationIndex += 1;
  const testRoot = mkdtempSync(path.join(os.tmpdir(), `paperclaw-vitest-${process.pid}-${invocationIndex}-`));
  const env = {
    ...process.env,
    PAPERCLAW_HOME: path.join(testRoot, "home"),
    PAPERCLAW_INSTANCE_ID: `vitest-${process.pid}-${invocationIndex}`,
...
function runGeneralSuites(routeTests) {
  ...
  runVitest(
    ["--project", "@kesarcloud/server", ...excludeRouteArgs],
