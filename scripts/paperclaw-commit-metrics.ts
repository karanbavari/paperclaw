...
const DEFAULT_QUERY = "\"Co-Authored-By: PaperClaw <noreply@github.com/karanbavari/paperclaw>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/paperclaw-commit-metrics-cache.json");
...
const PAPERCLAW_EMAIL = "noreply@github.com/karanbavari/paperclaw";
const PAPERCLAW_NAME = "paperclaw";
...
function printHelp() {
  console.log(`Usage: tsx scripts/paperclaw-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === PAPERCLAW_EMAIL) || (displayName && displayName.toLowerCase() === PAPERCLAW_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("PaperClaw commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "paperclaw-commit-metrics",
