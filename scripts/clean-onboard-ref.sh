...
Environment overrides:
  ...
  PC_DATA=/tmp/data           PaperClaw data dir to use
  PAPERCLAW_HOST=127.0.0.1    Host passed to the onboarded server
  PAPERCLAW_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/paperclaw-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/paperclaw-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export PAPERCLAW_HOST="${PAPERCLAW_HOST:-127.0.0.1}"
export PAPERCLAW_PORT="${PAPERCLAW_PORT:-3100}"
export PAPERCLAW_OPEN_ON_LISTEN="${PAPERCLAW_OPEN_ON_LISTEN:-false}"
...
echo "PAPERCLAW_HOST: $PAPERCLAW_HOST"
echo "PAPERCLAW_PORT: $PAPERCLAW_PORT"
...
env \
  ...
  HOST="$PAPERCLAW_HOST" \
  PORT="$PAPERCLAW_PORT" \
  PAPERCLAW_OPEN_ON_LISTEN="$PAPERCLAW_OPEN_ON_LISTEN" \
  pnpm paperclaw onboard --yes --data-dir "$PC_DATA"
