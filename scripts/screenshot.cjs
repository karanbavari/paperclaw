#!/usr/bin/env node
/**
 * Screenshot utility for PaperClaw UI.
 *
 * Reads the board token from ~/.paperclaw/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/paperclaw-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".paperclaw/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.paperclaw/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the PaperClaw origin only
