...
PAPERCLAW_HOST_PORT="${PAPERCLAW_HOST_PORT:-3100}"
PAPERCLAW_HOST_FROM_CONTAINER="${PAPERCLAW_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.paperclaw-openclaw.override.yml"
...
detect_paperclaw_base_url() {
  ...
  for candidate in "$PAPERCLAW_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${PAPERCLAW_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${PAPERCLAW_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
paperclaw_base_url="$(detect_paperclaw_base_url || true)"
...
PaperClaw URL for OpenClaw container:
EOF
  if [[ -n "$paperclaw_base_url" ]]; then
    cat <<EOF
  $paperclaw_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${PAPERCLAW_HOST_PORT}
  (Do not use http://127.0.0.1:${PAPERCLAW_HOST_PORT} inside the container.)
  If PaperClaw rejects the host, run on host machine:
    pnpm paperclaw allowed-hostname host.docker.internal
  Then restart PaperClaw and re-run this script.
EOF
  fi
  cat <<EOF
