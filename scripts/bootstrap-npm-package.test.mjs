...
test("parseArgs recognizes publish and skip-build flags", () => {
  assert.deepEqual(parseArgs(["@kesarcloud/adapter-acpx-local", "--publish", "--skip-build"]), {
    help: false,
    selector: "@kesarcloud/adapter-acpx-local",
    publish: true,
    skipBuild: true,
    otp: null,
  });
...
test("resolveTargetPackage matches by package name or dir", () => {
  const packages = [
    { dir: "packages/a", name: "@kesarcloud/a", pkg: {} },
    { dir: "packages/b", name: "@kesarcloud/b", pkg: {} },
  ];

  assert.equal(resolveTargetPackage("@kesarcloud/a", packages).dir, "packages/a");
  assert.equal(resolveTargetPackage("./packages/b", packages).name, "@kesarcloud/b");
});
