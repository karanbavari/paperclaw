# PaperClaw V1 Implementation Spec
...
- `runtime_config` jsonb not null default `{}`; may include PaperClaw runtime policy such as `modelProfiles.cheap.adapterConfig` for an optional low-cost model lane that does not change the primary adapter config
...
- project env is merged into run environment for issues in that project and overrides conflicting agent env keys before PaperClaw runtime-owned keys are injected
...
- when PaperClaw cannot safely infer the next action, it surfaces the problem through visible blocked/recovery work instead of silently completing or reassigning work
...
- `.paperclaw.yaml` sidecar for PaperClaw-specific fidelity
...
- recurring `TASK.md` entries use `recurring: true` in the base package and PaperClaw routine fidelity in `.paperclaw.yaml`
- PaperClaw imports recurring task packages as routines instead of downgrading them to one-time issues
