...
Maintainer runbook for shipping PaperClaw across npm, GitHub, and the website-facing changelog surface.
...
PaperClaw uses calendar versions that still fit semver syntax:
...
[Run the action here](https://github.com/karanbavari/paperclaw/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for PaperClaw. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
gh workflow run release-smoke.yml -f paperclaw_version=canary
gh workflow run release-smoke.yml -f paperclaw_version=latest
```
