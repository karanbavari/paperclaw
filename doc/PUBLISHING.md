...
Low-level reference for how PaperClaw packages are prepared and published to npm.
...
PaperClaw no longer uses release branches or Changesets for publishing.
...
PaperClaw uses calendar versions:
...
PaperClaw does not auto-publish every non-private workspace package anymore.
...
Example for `@kesarcloud/adapter-acpx-local` from the repo root:

```bash
# safe preview
pnpm run release:bootstrap-package -- @kesarcloud/adapter-acpx-local

# one-time first publish from an authenticated maintainer machine
pnpm run release:bootstrap-package -- @kesarcloud/adapter-acpx-local --publish --otp 123456
```
...
1. open `https://www.npmjs.com/package/@kesarcloud/adapter-acpx-local`
2. go to `Settings` → `Trusted publishing`
3. add repository `karanbavari/paperclaw`
