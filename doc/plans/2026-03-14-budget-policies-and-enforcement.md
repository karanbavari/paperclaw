...
PaperClaw already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model PaperClaw should implement next.

## Product Goals

PaperClaw should let operators:
...
For the next implementation pass, PaperClaw should enforce these defaults:
...
PaperClaw should separate subscription-included usage from billed spend:
...
PaperClaw should have two threshold classes:
...
PaperClaw must know that a project is budget-paused and enforce it.
...
PaperClaw must also block execution before new work starts.
