...
`PAP-447` asks how PaperClaw should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same PaperClaw instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into PaperClaw:

- some operators use PaperClaw to manage PaperClaw and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as PaperClaw
- Claude and Codex expose different built-in affordances, so PaperClaw should not overfit to one tool

## Core Product Decision

PaperClaw should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by PaperClaw
...
- PaperClaw should not assume that every adapter can see or use a host filesystem path directly
...
If PaperClaw makes worktrees agent-first, it will blur:
...
- PaperClaw core resolves and records execution workspace state
...
That gives PaperClaw room to support:
...
- PaperClaw handles workspace runtime services under the hood using project/workspace policy
...
Once PaperClaw is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So PaperClaw should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that PaperClaw cannot infer
...
- when issue moves to done, should PaperClaw auto-commit?
- should it auto-push?
...
PaperClaw already has the right foundation for a project-first model.
...
For server-spawned local adapters, PaperClaw already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that PaperClaw should mirror directly

Implication:

- for `codex_local`, PaperClaw should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but PaperClaw should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, PaperClaw should separate:
...
PaperClaw should be able to express intentions such as:
...
These are not specific to PaperClaw. They are a common property of working in a dev workspace, whether local or remote.
...
Do not model this as a PaperClaw-specific `devServer` flag.
...
PaperClaw should distinguish between:
...
PaperClaw should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, PaperClaw should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to PaperClaw
...
For runtime services specifically, PaperClaw should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as PaperClaw-only dev-server flags
...
- same embedded DB / PaperClaw instance can be reused through env/config injection when appropriate
...
- PaperClaw can decide whether to reuse or start a fresh service deterministically
...
3. Test against PaperClaw-on-PaperClaw workflows first.
...
7. Workspace runtime services are modeled generically, not as PaperClaw-specific dev-server toggles.
