...
PaperClaw's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends PaperClaw's API, UI, and data model | Yes |
...
modules/
  observability/
    paperclaw.module.json     # manifest (required)
    src/
...
### Manifest (`paperclaw.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@kesarcloud/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@kesarcloud/core";
...
```
1. Scan modules/ directory for paperclaw.module.json manifests
...
```jsonc
// paperclaw.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@kesarcloud/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@kesarcloud/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:paperclaw/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.github.com/karanbavari/paperclaw/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm paperclaw store list                    # browse available modules and templates
pnpm paperclaw store install <module-id>     # install a module
pnpm paperclaw store import <template-id>    # import a company template
pnpm paperclaw store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one PaperClaw instance | `server:started` |
...
4. **Module config** — `paperclaw.config.json` with per-module config, env var interpolation
...
10. **`@kesarcloud/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
