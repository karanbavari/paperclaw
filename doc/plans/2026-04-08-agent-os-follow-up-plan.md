...
Turn the `agent-os` research into a low-risk PaperClaw execution plan that preserves PaperClaw's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

PaperClaw should not absorb `agent-os` as a product model or orchestration layer.

PaperClaw should evaluate `agent-os` in three narrow areas:
...
This work is successful when PaperClaw has:
...
- a proposed runtime capability model that fits current PaperClaw adapters
...
- replace PaperClaw heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to PaperClaw core

## Existing PaperClaw integration points
...
- prove that PaperClaw can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep PaperClaw's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a PaperClaw-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some PaperClaw workloads
...
- identify where PaperClaw should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in PaperClaw
...
- runtime experimentation could blur the boundary between PaperClaw as control plane and PaperClaw as execution platform
