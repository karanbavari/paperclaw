...
Add a single storage subsystem for PaperClaw that supports:
...
- API serves attachment bytes through authenticated PaperClaw endpoints.
...
- Default local path is under instance root: `~/.paperclaw/instances/<instanceId>/data/storage`.
...
- `paperclaw onboard` writes a valid `storage` config block by default.
- `paperclaw configure --section storage` can switch between local and s3 modes.
...
- `paperclaw doctor` reports actionable storage status.
