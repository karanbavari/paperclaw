...
This document defines the V1 design for a PaperClaw adapter that integrates with
...
4. synthesized stdout events for PaperClaw UI/CLI
...
3. **Agent callback auth to PaperClaw** uses a bootstrap exchange flow (no long-lived PaperClaw key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from PaperClaw endpoints, not full SKILL.md prompt injection.
...
- `paperclawPublicUrl` (optional override; else `PAPERCLAW_PUBLIC_URL` env)
...
## PaperClaw Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `PAPERCLAW_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.paperclawPublicUrl`
2. `process.env.PAPERCLAW_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived PaperClaw credentials in prompt text.

Flow:

1. Before launch/follow-up, PaperClaw mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. PaperClaw validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all PaperClaw API calls.
...
1. Prompt includes a compact instruction to fetch skills from PaperClaw.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/paperclaw`
   - `GET /api/skills/paperclaw-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <paperclawPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active PaperClaw run context.
...
5. `paperclawPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from PaperClaw skill endpoints |
| PaperClaw auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from PaperClaw context.
3. Add richer PR metadata surfacing in PaperClaw UI.
