...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented PaperClaw plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside PaperClaw.
...
- package: `@kesarcloud/plugin-kitchen-sink-example`
- plugin id: `paperclaw.kitchen-sink-example` or `paperclaw-kitchen-sink-example`

Recommendation: use `paperclaw-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core PaperClaw event such as `issue.created`
...
Recommendation: default to a PaperClaw-local endpoint or a stable public echo endpoint to avoid flaky docs.
