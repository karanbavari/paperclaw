...
- local mode that still feels instant (`npx @kesarcloud/paperclaw run` and go)
...
2. CLI command `pnpm paperclaw auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm paperclaw onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm paperclaw onboard`).
...
- expose key instance settings in API + CLI (`paperclaw configure` / `paperclaw onboard`)
...
7. `pnpm paperclaw onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
