...
- making PaperClaw feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing PaperClaw users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted PaperClaw deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning PaperClaw into a full code review product
...
This object must be recorded explicitly so that PaperClaw can:
...
PaperClaw should treat PRs as a type of work product linked back to:
...
PaperClaw already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted PaperClaw must be a first-class deployment mode

PaperClaw cannot assume that it is running on the same machine as the code.

In cloud deployments, PaperClaw may:
...
- `Runtime service`: a process or service PaperClaw owns or tracks for a workspace
...
It also lets cloud-hosted PaperClaw deployments point at codebases and remotes without pretending the PaperClaw host has direct filesystem access.
...
Without an explicit `execution workspace` record, PaperClaw has nowhere stable to attach:
...
PaperClaw needs a single place to show:
...
- PaperClaw-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without PaperClaw owning the execution host
...
## 1. PaperClaw host and execution host must be decoupled
...
without the PaperClaw host itself running local git or local preview processes.
...
PaperClaw should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for PaperClaw.
...
PaperClaw should keep workspace automation centered on a small set of hooks:
...
PaperClaw should provide the equivalent visibility in the project `Code` surface:
...
PaperClaw should treat execution workspaces as the natural home for:
...
PaperClaw should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
PaperClaw should continue treating cleanup policy as part of the core workspace model:
...
PaperClaw should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the PaperClaw host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

PaperClaw should borrow Worktrunk's good execution patterns while keeping the broader PaperClaw model:
...
This is how PaperClaw supports:
...
That keeps PaperClaw focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted PaperClaw deployments remain first-class

That is a better fit for PaperClaw than either extreme:
