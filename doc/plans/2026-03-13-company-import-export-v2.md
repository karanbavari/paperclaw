...
This document defines the next-stage plan for PaperClaw company import/export.

The core shift is:

- move from a PaperClaw-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside PaperClaw.
...
PaperClaw already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just PaperClaw
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `paperclaw.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.paperclaw.yaml` sidecar for high-fidelity PaperClaw-specific details
...
11. adapter-aware skill sync surfaces so PaperClaw can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point PaperClaw at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a PaperClaw sidecar.
...
PaperClaw must not redefine `SKILL.md`.
...
- PaperClaw-specific fidelity lives in `.paperclaw.yaml`
- PaperClaw may resolve and install `SKILL.md` packages, but it must not require a PaperClaw-only skill format
...
### 5.4 Base Package Vs PaperClaw Extension
...
- base package:
  ...
  - no PaperClaw-only runtime fields by default
- PaperClaw extension:
  - `.paperclaw.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by PaperClaw tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`paperclaw.manifest.json` is not part of the future package direction.
...
PaperClaw import/export should support these entity kinds:
...
- PaperClaw can treat “this set of agents came from team package X” as the imported-team model
...
PaperClaw should support source references in package metadata with:
...
PaperClaw should define a new adapter surface area around skills:
...
Planned PaperClaw behavior:

- if an adapter supports read, PaperClaw should show current skills in the UI
- if an adapter supports write, PaperClaw should let the user enable/disable imported skills
- if an adapter supports sync, PaperClaw should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.paperclaw.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.paperclaw.yaml` alongside the base package
...
## 10. Storage Model Inside PaperClaw
...
PaperClaw should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `paperclaw company export <company-id> --out <path>`
- `paperclaw company import <path-or-url> --dry-run`
- `paperclaw company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in PaperClaw storage, or only referenced at import time?
...
3. Should PaperClaw support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps PaperClaw aligned with:
