...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/paperclaw-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `PAPERCLAW_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `PAPERCLAW_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - PaperClaw may create host-local worktrees, processes, and paths
- `cloud`
  - PaperClaw should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `paperclaw | github | vercel | s3 | custom`
...
- use `PAPERCLAW_EXECUTION_TOPOLOGY` as a defaulting guardrail
