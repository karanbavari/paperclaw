...
Yes, there is a likely root cause in PaperClaw's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, PaperClaw can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: PaperClaw already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- PaperClaw may think it cancelled the run because the parent process is gone
...
This work is successful when PaperClaw can:
...
- require every skill author to add bespoke browser teardown logic before PaperClaw can clean up correctly
...
- make the leak measurable from PaperClaw's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by PaperClaw
...
Treat this as a PaperClaw executor ownership bug, not an `agent-browser` bug.
