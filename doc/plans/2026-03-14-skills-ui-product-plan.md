...
This document defines the product and UI plan for skill management in PaperClaw.
...
- PaperClaw company import/export can include skills as package content
...
3. The UI must always tell the truth about what PaperClaw knows:
   - desired state in PaperClaw
...
PaperClaw should treat skills at two scopes:
...
- created locally inside PaperClaw later
...
7. “Understand what PaperClaw knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a PaperClaw-specific package layout
...
This matters because Codex and similar adapters may already have local skills that PaperClaw did not install.

These should be clearly marked:

- external
- not managed by PaperClaw
...
- `desired_only`: PaperClaw wants it, adapter does not show it yet
- `external`: adapter has it but PaperClaw does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, PaperClaw only tracks desired state
...
- PaperClaw can still track desired skills
...
- PaperClaw can see adapter skills
...
- `.paperclaw.yaml` may add PaperClaw-specific fidelity, but should not replace the base shortname association model
...
- `.paperclaw.yaml` may add PaperClaw-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
PaperClaw should have a company-scoped skill model or managed package model representing:
...
PaperClaw should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too PaperClaw-specific.
...
That gives PaperClaw one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
