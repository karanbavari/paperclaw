...
Agents need API keys to authenticate with PaperClaw. The current approach
...
**Trust model:** The adapter process runs on the same machine as the PaperClaw
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** PaperClaw generates a token and passes it directly to the agent
...
`PAPERCLAW_API_URL`, `PAPERCLAW_AGENT_ID`, `PAPERCLAW_COMPANY_ID`. We need to
add a `PAPERCLAW_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a PaperClaw CLI
...
```
paperclaw auth login
# Opens browser -> user confirms -> token stored at ~/.paperclaw/credentials
```
...
managed by the PaperClaw server itself.
...
1. A company admin (user or agent) generates an **invite URL** from PaperClaw.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The PaperClaw SKILL.md (or a link to it)
   - What information PaperClaw needs from the agent (e.g. webhook URL, adapter
...
5. PaperClaw stores the pending registration.
...
7. On approval, PaperClaw provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** PaperClaw issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.github.com/karanbavari/paperclaw/skills/paperclaw/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How PaperClaw should send you heartbeats",
...
1. Generate a PaperClaw invite link for the company.
...
4. A PaperClaw company member approves the new agent.
5. PaperClaw begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `PAPERCLAW_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `paperclaw auth login` for developer-managed remote agents.                                      |
