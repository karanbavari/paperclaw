...
This document explains how PaperClaw interprets issue assignment, issue status, execution runs, wakeups, parent/sub-issue structure, and blocker relationships.

`doc/SPEC-implementation.md` remains the V1 contract. This document is the detailed execution model behind that contract.

## 1. Core Model

PaperClaw separates four concepts that are easy to blur together:
...
This is a hard invariant. PaperClaw is single-assignee by design.

## 3. Status Semantics

PaperClaw issue statuses are not just UI labels. They imply different expectations about ownership and execution.
...
- for agent-assigned work, PaperClaw may still need a wake path to ensure the assignee actually sees it
...
- waiting on an external dependency or system when PaperClaw does not own a scheduled re-check
...
- PaperClaw can wake the assignee
- PaperClaw can track runs linked to the issue
- PaperClaw can recover some lost execution state after crashes/restarts

### User-owned issues

User-owned issues are not executed by the heartbeat scheduler.

- PaperClaw can track the ownership and status
- PaperClaw cannot rely on heartbeat/run semantics to keep them moving
...
PaperClaw already clears stale execution locks and can adopt some stale checkout locks when the original run is gone.

## 6. Parent/Sub-Issue vs Blockers

PaperClaw uses two different relationships for different jobs.
...
Blocked issues should stay idle while blockers remain unresolved. PaperClaw should not create a queued heartbeat run for that issue until the final blocker is done and the `issue_blockers_resolved` wake can start real work.

If a parent is truly waiting on a child, model that with blockers. Do not rely on the parent/child relationship alone.

## 7. Non-Terminal Issue Liveness Contract

For agent-owned, non-terminal issues, PaperClaw should never leave work in a state where nobody is responsible for the next move and nothing will wake or surface it.

This is a visibility contract, not an auto-completion contract. If PaperClaw cannot safely infer the next action, it should surface the ambiguity with a blocked state, a visible comment, or an explicit recovery issue. It must not silently mark work done from prose comments or guess that a dependency is complete.
...
An `in_review` issue is stalled when it has no typed participant, no pending interaction or approval, no user owner, no active monitor, no active run, no queued wake, and no explicit recovery issue. PaperClaw should surface that state as recovery work rather than silently completing the issue or leaving blocker chains parked indefinitely.
...
- `nextCheckAt`: when PaperClaw should wake the assignee
- `notes`: non-secret instructions for what the assignee should check
- `serviceName`: optional non-secret external-service context
- `externalRef`: optional external-service reference input; PaperClaw treats it as secret-adjacent, redacts it before persistence/visibility, and omits it from activity and wake payloads
- `timeoutAt`, `maxAttempts`, and `recoveryPolicy`: optional recovery hints for bounded waits

Monitors are not recurring intervals. When a monitor fires, PaperClaw clears the scheduled monitor and queues an `issue_monitor_due` wake for the assignee. If the external service is still pending, the assignee must explicitly re-arm the monitor with a new `nextCheckAt`. If the issue moves to `done`, `cancelled`, an invalid status, or a human/unassigned owner, the monitor is cleared.
...
Monitor bounds are enforced. PaperClaw rejects attempts to re-arm a monitor whose `timeoutAt` or `maxAttempts` is already exhausted. When a scheduled monitor reaches an exhausted bound at trigger time, PaperClaw clears it and follows `recoveryPolicy`: `wake_owner` queues a bounded recovery wake for the assignee, `create_recovery_issue` opens visible recovery work, and `escalate_to_board` records a board-visible escalation comment/activity.

Use `blocked` instead of a monitor when no PaperClaw assignee owns a responsible polling path. In that case, name the external owner/action or create first-class recovery/blocker work.
...
PaperClaw now treats crash/restart recovery as a stranded-assigned-work problem, not just a stranded-run problem.
...
- if the latest issue-linked run failed/timed out/cancelled and no live execution path remains, PaperClaw queues one automatic assignment recovery wake
- if that recovery wake also finishes and the issue is still stranded, PaperClaw moves the issue to `blocked` and posts a visible comment
...
- PaperClaw queues one automatic continuation wake
- if that continuation wake also finishes and the issue is still stranded, PaperClaw moves the issue to `blocked` and posts a visible comment
...
On startup and on the periodic recovery loop, PaperClaw now does four things in sequence:
...
An active run can still be unhealthy even when its process is `running`. PaperClaw treats prolonged output silence as a watchdog signal, not as proof that the run is failed.
...
PaperClaw uses three different recovery outcomes, depending on how much it can safely infer.
...
PaperClaw creates an explicit recovery issue when the system can identify a problem but cannot safely complete the work itself.
...
In these cases PaperClaw should leave a visible issue/comment trail instead of silently retrying.

## 12. What This Does Not Mean

These semantics do not change V1 into an auto-reassignment system.

PaperClaw still does not:
...
That is the execution contract PaperClaw should present to operators.
