...
For `claude_local` and `codex_local`, PaperClaw assumes the CLI is already installed and authenticated on the host machine.
...
PaperClaw stores resumable session state per `(agent, taskKey, adapterType)`.
