...
ClipHub is the public registry where people share, discover, and download PaperClaw company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to PaperClaw what a package registry is to a programming language. PaperClaw already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in PaperClaw — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own PaperClaw instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the PaperClaw spec. It contains:
...
**Install (fresh start):**
```
paperclaw install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local PaperClaw instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within PaperClaw, export your company as a template, then publish:

```
paperclaw export --template my-company
paperclaw publish cliphub my-company
```
...
ClipHub is a **separate service** from PaperClaw itself. PaperClaw is self-hosted; ClipHub is a hosted registry that PaperClaw instances talk to.
...
| **PaperClaw CLI** | `paperclaw install`, `paperclaw publish`, `paperclaw cliphub sync` — built into PaperClaw |
| **PaperClaw UI** | "Browse ClipHub" panel in the PaperClaw web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with PaperClaw) |
| Backend | TypeScript + Hono (consistent with PaperClaw) |
| Database | PostgreSQL |
...
4. Run `paperclaw install cliphub:acme/lean-saas-shop`
5. PaperClaw creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in PaperClaw until it works well
2. Export: `paperclaw export --template my-agency`
3. Publish: `paperclaw publish cliphub my-agency`
...
3. Install just that agent: `paperclaw install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to PaperClaw

ClipHub is **not required** to use PaperClaw. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to PaperClaw what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `paperclaw install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in PaperClaw web UI
- [ ] `paperclaw cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with PaperClaw's own philosophy)
