...
PaperClaw V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a PaperClaw-shaped design.

Assumption for this document: PaperClaw is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class PaperClaw objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into PaperClaw.

The main conclusion is:

- PaperClaw should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- PaperClaw should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- PaperClaw should use multiple extension classes instead of one generic plugin bag:
...
If PaperClaw does this well, the examples you listed become straightforward:
...
Relevant PaperClaw files reviewed for current extension seams:
...
It is too dangerous for PaperClaw core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for PaperClaw — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
PaperClaw should absolutely do this.
...
PaperClaw should copy this discipline.
...
PaperClaw should adopt this style for plugin actions, automations, and UI schemas.
...
PaperClaw can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for PaperClaw too.

## What PaperClaw Should Not Copy Directly
...
PaperClaw is a control plane for an operator-managed instance with company objects.
...
PaperClaw is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

PaperClaw should not auto-load arbitrary code from a workspace repo like `.paperclaw/plugins` or project directories.
...
For PaperClaw, equivalent hooks into:
...
PaperClaw should not allow plugins to silently replace:
...
For PaperClaw it would be risky because it combines:
...
PaperClaw should require an explicit operator install step.

## Why PaperClaw Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | PaperClaw |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted PaperClaw instance |
...
That means PaperClaw should borrow the good ideas from `opencode` but use a stricter architecture.

## PaperClaw Already Has Useful Pre-Plugin Seams

PaperClaw has several extension-like seams already:
...
This is good news.
PaperClaw does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended PaperClaw Plugin Model
...
PaperClaw already has this pattern implicitly:
...
- `plugin` means integration code that talks to PaperClaw through a typed plugin protocol and capability model
...
- invoke explicit PaperClaw actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@kesarcloud/plugin-sdk/ui`) with design tokens and common components.
...
PaperClaw should treat plugin installation as a global instance-level action.

Examples:

- install `@kesarcloud/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over PaperClaw objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

PaperClaw already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for PaperClaw too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@kesarcloud/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@kesarcloud/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running PaperClaw instance.

A `create-paperclaw-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how PaperClaw achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the PaperClaw server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. PaperClaw's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@kesarcloud/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@kesarcloud/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For PaperClaw

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@kesarcloud/plugin-sdk";

export default definePlugin({
  id: "@kesarcloud/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit PaperClaw entities
    });
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@kesarcloud/plugin-sdk/ui";
...
Any PaperClaw plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific PaperClaw objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@kesarcloud/plugin-linear`)
...
- reuse core tables when the data is clearly part of PaperClaw itself
...
If a concept is becoming part of PaperClaw's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of PaperClaw itself
...
### 3. Scope plugin data to PaperClaw objects before adding custom schemas

A lot of plugin data naturally hangs off existing PaperClaw objects:
...
If PaperClaw eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@kesarcloud/plugin-workspace-files`
...
Package idea: `@kesarcloud/plugin-terminal`
...
Package idea: `@kesarcloud/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving PaperClaw.
- Board reviews the diff after a run without leaving PaperClaw.
...
- optional `events.emit` (e.g. `plugin.@kesarcloud/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@kesarcloud/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@kesarcloud/plugin-linear`

This plugin syncs PaperClaw work with Linear. It is useful for:

- importing backlog from Linear
- linking PaperClaw issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between PaperClaw and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into PaperClaw.
- PaperClaw issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@kesarcloud/plugin-github-issues`

This plugin syncs PaperClaw issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to PaperClaw issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with PaperClaw task governance
...
- Plugin settings:
  ...
  - whether new PaperClaw issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active PaperClaw issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into PaperClaw.
- GitHub webhooks update status/comment state in PaperClaw.
- A PR is linked back to the PaperClaw issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@kesarcloud/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@kesarcloud/plugin-grafana`

This plugin surfaces external metrics and dashboards inside PaperClaw. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating PaperClaw issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full PaperClaw plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the PaperClaw dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a PaperClaw issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of PaperClaw core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@kesarcloud/plugin-runtime-processes`
...
Package idea: `@kesarcloud/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into PaperClaw. It is useful for:
...
- Failed payment spikes or churn events can generate PaperClaw issues for follow-up.
...
- Stripe data should stay additive to PaperClaw core
...
- plugin UI bundle loading, host bridge, `@kesarcloud/plugin-sdk/ui`
...
- `@kesarcloud/plugin-test-harness` and `create-paperclaw-plugin` starter template
...
PaperClaw should not implement "an OpenCode-style generic in-process hook system."
PaperClaw should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In PaperClaw
...
8. Ship `@kesarcloud/plugin-test-harness` and `create-paperclaw-plugin` starter template.
