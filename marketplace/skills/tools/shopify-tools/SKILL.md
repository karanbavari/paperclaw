...
- Use `shopify.ordersSearch` and `shopify.orderGet` for order support, fulfillment investigation, and reporting. Do not expose sensitive customer details unless the issue requires it.
...
- If Shopify returns userErrors, include the field and message in the issue comment and stop until the request is corrected.

## Handoff Notes

When a Shopify task is complete, comment with:

- shop domain
- tools used
- whether the run was dry-run or live
- concrete result or Shopify userErrors
- next action, if any
