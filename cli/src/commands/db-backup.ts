...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@kesarcloud/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://paperclaw:paperclaw@127.0.0.1:${port}/paperclaw`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" paperclaw db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolvePaperClawInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "paperclaw";
