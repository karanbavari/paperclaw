...
import { ensureAgentJwtSecret, loadPaperClawEnvFile, mergePaperClawEnvEntries, readPaperClawEnvEntries, resolvePaperClawEnvFile } from "../config/env.js";
...
const WORKTREE_NAME_PREFIX = "paperclaw-";
...
function createConfiguredStorageFromPaperClawConfig(config: PaperClawConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromPaperClawConfig(config);
}
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  const envEntries = readPaperClawEnvEntries(resolvePaperClawEnvFile(input.configPath));
  ...
  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local PaperClaw instance. Expected PAPERCLAW_HOME and PAPERCLAW_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
function resolveExistingGitWorktree(selector: string, cwd: string): MergeSourceChoice | null {
  ...
  if (existsSync(directPath)) {
    return {
      ...
      hasPaperClawConfig: existsSync(path.resolve(directPath, ".paperclaw", "config.json")),
...
function resolveSourceConnectionString(config: PaperClawConfig, envEntries: Record<string, string>, portOverride?: number): string {
...
async function seedWorktreeDatabase(input: {
  ...
  const sourceEnvFile = resolvePaperClawEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readPaperClawEnvEntries(sourceEnvFile);
  ...
  try {
    if (input.sourceConfig.database.mode === "embedded-postgres") {
      ...
      const sourceAdminConnectionString = `postgres://paperclaw:paperclaw@127.0.0.1:${sourceHandle.port}/postgres`;
      await ensurePostgresDatabase(sourceAdminConnectionString, "paperclaw");
    }
    ...
    const adminConnectionString = `postgres://paperclaw:paperclaw@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "paperclaw");
    const targetConnectionString = `postgres://paperclaw:paperclaw@127.0.0.1:${targetHandle.port}/paperclaw`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readPaperClawEnvEntries(resolvePaperClawEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.PAPERCLAW_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.PAPERCLAW_AGENT_JWT_SECRET);
  mergePaperClawEnvEntries(
    {
  ...
  loadPaperClawEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run PaperClaw inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" paperclaw worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printPaperClawCliBanner();
  p.intro(pc.bgCyan(pc.black(" paperclaw worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasPaperClawConfig: existsSync(path.resolve(worktreePath, ".paperclaw", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printPaperClawCliBanner();
  p.intro(pc.bgCyan(pc.black(" paperclaw worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolvePaperClawEnvFile(configPath);
  const envEntries = readPaperClawEnvEntries(envPath);
  const out = {
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasPaperClawConfig)
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readPaperClawEnvEntries(resolvePaperClawEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start PaperClaw once) before using worktree merge history.`,
      );
    }
...
function resolveRunningEmbeddedPostgresPid(config: PaperClawConfig): number | null {
  if (config.database.mode !== "embedded-postgres") {
    return null;
  }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasPaperClawConfig ? "paperclaw" : "no-paperclaw-config",
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (!matched.hasPaperClawConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a PaperClaw worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasPaperClawConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No PaperClaw worktrees were found. Run `paperclaw worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target PaperClaw configs are the same. Choose different --from/--to worktrees.");
  }
...
async function runWorktreeReseed(opts: WorktreeReseedOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target PaperClaw configs are the same. Choose different --from/--to values.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop PaperClaw in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Overwrite the isolated PaperClaw DB for ${targetEndpoint.label} from ${source.label} using ${seedMode} seed mode?`,
      initialValue: false,
    });
  if (p.isCancel(confirmed) || !confirmed) {
...
export async function worktreeRepairCommand(opts: WorktreeRepairOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(target.configPath)) {
    throw new Error("Source and target PaperClaw configs are the same. Use --from-config/--from-instance to point repair at a different source.");
  }

  const targetConfig = existsSync(target.configPath) ? readConfig(target.configPath) : null;
  const targetEnvEntries = readPaperClawEnvEntries(resolvePaperClawEnvFile(target.configPath));
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop PaperClaw in ${target.rootPath} before repairing, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  if (runningTargetPid && opts.allowLiveTarget) {
    p.log.warning(`Proceeding even though the target embedded PostgreSQL appears to be running (pid ${runningTargetPid}).`);
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local PaperClaw instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated PaperClaw instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with paperclaw- if needed (created at ~/paperclaw-NAME)")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local PaperClaw instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like PaperClaw worktrees")
  ...
  worktree
    .command("reseed")
    .description("Re-seed an existing worktree-local instance from another PaperClaw instance or worktree")
    .option("--from <worktree>", "Source worktree path, directory name, branch name, or current")
    .option("--to <worktree>", "Target worktree path, directory name, branch name, or current (defaults to current)")
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source PAPERCLAW_HOME used when deriving the source config")
  ...
  worktree
    .command("repair")
    .description("Create or repair a linked worktree-local PaperClaw instance without touching the primary checkout")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with paperclaw- if needed")
