...
export async function doctor(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" paperclaw doctor ")));
  ...
  try {
    ...
    const readResult: CheckResult = {
      ...
      canRepair: false,
      repairHint: "Run `paperclaw configure --section database` or `paperclaw onboard`",
    };
...
async function runRepairableCheck(input: {
  ...
  loadPaperClawEnvFile(input.configPath);
