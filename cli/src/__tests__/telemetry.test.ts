...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "paperclaw-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.PAPERCLAW_HOME = path.join(root, "home");
    process.env.PAPERCLAW_INSTANCE_ID = "telemetry-test";
