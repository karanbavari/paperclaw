...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "paperclaw-onboard-"));
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "paperclaw-onboard-fresh-"));
  return path.join(root, ".paperclaw", "config.json");
}
...
describe("onboard", () => {
  ...
  it("keeps --yes onboarding on local trusted loopback defaults", async () => {
    ...
    process.env.PAPERCLAW_BIND = "lan";

    await onboard({ config: configPath, yes: true, invokedByRun: true });

    const raw = JSON.parse(fs.readFileSync(configPath, "utf8")) as PaperClawConfig;
