...
import { paperclawConfigSchema, type PaperClawConfig } from "./schema.js";
import {
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolvePaperClawInstanceId());
}
...
export function readConfig(configPath?: string): PaperClawConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = paperclawConfigSchema.safeParse(migrated);
  if (!parsed.success) {
