...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by PaperClaw CLI commands",
...
export function resolvePaperClawEnvFile(configPath?: string): string {
...
export function loadPaperClawEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergePaperClawEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readPaperClawEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writePaperClawEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergePaperClawEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readPaperClawEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writePaperClawEnvEntries(next, filePath);
  return next;
}
