...
const DATA_DIR_OPTION_HELP =
  "PaperClaw data directory root (isolates state from ~/.paperclaw)";
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadPaperClawEnvFile(options.config);
...
program
  ...
  .option("--run", "Start PaperClaw immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your PaperClaw setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "paperclaw")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run PaperClaw")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the PaperClaw server API")
