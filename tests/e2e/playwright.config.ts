...
const PAPERCLAW_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "paperclaw-e2e-home-"));
...
export default defineConfig({
  ...
  webServer: {
    command: `pnpm paperclaw onboard --yes --run`,
    url: `${BASE_URL}/api/health`,
    // Always boot a dedicated throwaway instance for e2e so browser tests
    // never attach to the developer's active PaperClaw home/server.
    ...
    env: {
      ...
      PORT: String(PORT),
      PAPERCLAW_HOME,
      PAPERCLAW_INSTANCE_ID: "playwright-e2e",
