 ...
 * renderYamlBlock.
 */
function filterPaperClawYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://github.com/karanbavari/paperclaw) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm paperclaw company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [PaperClaw](https://github.com/karanbavari/paperclaw) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [PaperClaw](https://github.com/karanbavari/paperclaw) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  const effectiveFiles = useMemo(() => {
    ...
    const yamlPath = exportData.paperclawExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterPaperClawYaml(exportData.files[yamlPath], checkedFiles);
    }
