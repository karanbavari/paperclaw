...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal PaperClaw issue links", () => {
    expect(parseIssuePathIdFromPath("https://github.com/karanbavari/paperclaw/issues/1778")).toBeNull();
    expect(parseIssueReferenceFromHref("https://github.com/karanbavari/paperclaw/issues/1778")).toBeNull();
  });
  ...
  it("preserves absolute PaperClaw issue URLs so origin, port, and hash are not lost", () => {
