...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="PaperClaw API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "paperclawApiUrl",
                  String(config.paperclawApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "paperclawApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://paperclaw.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/paperclaw-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "paperclaw"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="paperclaw"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. PaperClaw persists a device key during onboarding so pairing approvals
