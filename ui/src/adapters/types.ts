...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@kesarcloud/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@kesarcloud/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@kesarcloud/adapter-utils").TranscriptEntry[];
