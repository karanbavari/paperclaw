...
function MarkdownIssueLink({
  ...
  return (
    <Link
      ...
      className="paperclaw-markdown-issue-ref"
      title={title}
...
function createWikiLinkNode(href: string, wikiLink: ParsedWikiLink): MarkdownAstNode {
  return {
    ...
    data: {
      hProperties: {
        "data-paperclaw-wiki-link": "true",
        "data-paperclaw-wiki-target": wikiLink.target,
      },
    },
...
function CodeBlock({
  ...
  return (
    <div className="paperclaw-markdown-codeblock">
      <pre
        {...preProps}
        ref={preRef}
      ...
      <button
        ...
        title={label}
        className="paperclaw-markdown-codeblock-copy"
      ...
      >
        ...
        <span className="paperclaw-markdown-codeblock-copy-label">{label}</span>
      </button>
    </div>
  );
}
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`paperclaw-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="paperclaw-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("paperclaw-mermaid-status", error && "paperclaw-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="paperclaw-mermaid-source">
...
export function MarkdownBody({
  ...
  const components: Components = {
    ...
    table: ({ node: _node, style: tableStyle, children: tableChildren, ...tableProps }) => (
      <div className="paperclaw-markdown-table-scroll" role="region" aria-label="Scrollable table" tabIndex={0}>
    ...
    a: ({ node: _node, href, style: linkStyle, children: linkChildren, ...anchorProps }) => {
      ...
      const isWikiLink = dataProps["data-paperclaw-wiki-link"] === "true";
        ...
        return (
          <a
            ...
            className={cn(
              ...
              `paperclaw-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "paperclaw-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "paperclaw-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
