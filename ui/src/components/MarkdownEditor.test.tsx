...
import { buildProjectMentionHref, buildSkillMentionHref } from "@kesarcloud/shared";
import {
...
describe("MarkdownEditor", () => {
  ...
  it("keeps mention queries active across spaces", () => {
    expect(findMentionMatch("Ping @PaperClaw App", "Ping @PaperClaw App".length)).toEqual({
      trigger: "mention",
      marker: "@",
      query: "PaperClaw App",
      atPos: 5,
      endPos: "Ping @PaperClaw App".length,
    });
  });
  ...
  async function openMentionMenuFor(
    ...
    const option = Array.from(document.body.querySelectorAll('button[type="button"]'))
      .find((node) => node.textContent?.includes("PaperClaw App")) as HTMLButtonElement | undefined;
  ...
  it("accepts mention selection from a touch tap", async () => {
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[@PaperClaw App](${buildProjectMentionHref("project-123", "#336699")}) `,
    );
