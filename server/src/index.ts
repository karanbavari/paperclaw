...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.PAPERCLAW_SECRETS_PROVIDER === undefined) {
    process.env.PAPERCLAW_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.PAPERCLAW_SECRETS_STRICT_MODE === undefined) {
    process.env.PAPERCLAW_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.PAPERCLAW_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.PAPERCLAW_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.PAPERCLAW_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.PAPERCLAW_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set PAPERCLAW_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@kesarcloud.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.PAPERCLAW_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://paperclaw:paperclaw@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "paperclaw");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://paperclaw:paperclaw@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "paperclaw");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: paperclaw");
    }
  
    const embeddedConnectionString = `postgres://paperclaw:paperclaw@127.0.0.1:${port}/paperclaw`;
  ...
  const configuredApiUrl = process.env.PAPERCLAW_API_URL?.trim() || runtimeApiUrl;
  ...
  process.env.PAPERCLAW_LISTEN_HOST = runtimeListenHost;
  process.env.PAPERCLAW_LISTEN_PORT = String(listenPort);
  process.env.PAPERCLAW_RUNTIME_API_URL = runtimeApiUrl;
  process.env.PAPERCLAW_RUNTIME_API_CANDIDATES_JSON = JSON.stringify(runtimeApiCandidates);
  process.env.PAPERCLAW_API_URL = configuredApiUrl;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.PAPERCLAW_OPEN_ON_LISTEN === "true") {
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "PaperClaw server failed to start");
    process.exit(1);
  });
}
