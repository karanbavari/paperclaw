...
import { getAdapterSessionManagement } from "@kesarcloud/adapter-utils";
import {
...
} from "@kesarcloud/adapter-acpx-local";
import {
...
} from "@kesarcloud/adapter-pi-local";
import {
...
const hermesListSkillsForPaperClaw = hermesListSkills as unknown as NonNullable<ServerAdapterModule["listSkills"]>;
const hermesSyncSkillsForPaperClaw = hermesSyncSkills as unknown as NonNullable<ServerAdapterModule["syncSkills"]>;
...
const hermesLocalAdapter: ServerAdapterModule = {
  ...
  execute: async (ctx) => {
    ...
    const explicitApiKey =
      typeof existingEnv.PAPERCLAW_API_KEY === "string" && existingEnv.PAPERCLAW_API_KEY.trim().length > 0;
    ...
    const authGuardPrompt = [
      "PaperClaw API safety rule:",
      "Use Authorization: Bearer $PAPERCLAW_API_KEY on every PaperClaw API request.",
      "Use X-PaperClaw-Run-Id: $PAPERCLAW_RUN_ID on every PaperClaw API request that writes or mutates data, including comments and issue updates.",
      "Never use a board, browser, or local-board session for PaperClaw API writes.",
    ].join("\n");
    ...
    const patchedConfig: Record<string, unknown> = {
      ...
      env: {
        ...existingEnv,
        ...(!explicitApiKey ? { PAPERCLAW_API_KEY: normalizedCtx.authToken } : {}),
        PAPERCLAW_RUN_ID: normalizedCtx.runId,
      },
    };
  ...
  listSkills: hermesListSkillsForPaperClaw,
  syncSkills: hermesSyncSkillsForPaperClaw,
  models: hermesModels,
  supportsLocalAgentJwt: true,
  supportsInstructionsBundle: false,
  requiresMaterializedRuntimeSkills: false,
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[paperclaw] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[paperclaw] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[paperclaw] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[paperclaw] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
