...
function jwtConfig() {
  const secret = process.env.PAPERCLAW_AGENT_JWT_SECRET?.trim() || process.env.BETTER_AUTH_SECRET?.trim();
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.PAPERCLAW_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.PAPERCLAW_AGENT_JWT_ISSUER ?? "paperclaw",
    audience: process.env.PAPERCLAW_AGENT_JWT_AUDIENCE ?? "paperclaw-api",
  };
}
