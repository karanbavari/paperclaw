...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@kesarcloud/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  paperclawUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).paperclawUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.paperclawUpgradeContext = context;
