...
import type { PaperClawConfig } from "@kesarcloud/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by PaperClaw worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  const configPath = resolvePaperClawConfigPath(overrideConfigPath);
...
function writeConfigFile(configPath: string, config: PaperClawConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as PaperClawConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: PaperClawConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: PaperClawConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as PaperClawConfig;
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: PaperClawConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as PaperClawConfig;
  } catch {
    return;
  }
