...
import { resolvePaperClawConfigPath, resolvePaperClawEnvPath } from "./paths.js";
...
function resolveAgentJwtSecretStatus(
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm paperclaw onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolvePaperClawEnvPath();
