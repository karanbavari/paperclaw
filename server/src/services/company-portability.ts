...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".paperclaw.yaml" || normalized === ".paperclaw.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const paperclaw = isPlainRecord(metadata?.paperclaw) ? metadata?.paperclaw as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.paperclawSkillKey)
    ?? asString(paperclaw?.skillKey)
    ?? asString(paperclaw?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "paperclaw_bundled") {
    return `karanbavari/paperclaw/${slug}`;
  }
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "paperclaw" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; PaperClaw will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .paperclaw.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[paperclawExtensionPath];
  if (selectedFiles.has(paperclawExtensionPath) && typeof extensionEntry === "string") {
    filtered[paperclawExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findPaperClawExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".paperclaw.yaml"] === "string") return ".paperclaw.yaml";
  if (typeof files[".paperclaw.yml"] === "string") return ".paperclaw.yml";
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "paperclaw_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "karanbavari/paperclaw",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/karanbavari/paperclaw/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "paperclaw_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.paperclawSkillKey = skill.key;
  metadata.paperclaw = {
    ...(isPlainRecord(metadata.paperclaw) ? metadata.paperclaw : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const paperclawExtensionPath = findPaperClawExtensionPath(normalizedFiles);
  const paperclawExtension = paperclawExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, paperclawExtensionPath) ?? "")
    : {};
  const paperclawCompany = isPlainRecord(paperclawExtension.company) ? paperclawExtension.company : {};
  const paperclawSidebar = normalizePortableSidebarOrder(paperclawExtension.sidebar);
  const paperclawAgents = isPlainRecord(paperclawExtension.agents) ? paperclawExtension.agents : {};
  const paperclawProjects = isPlainRecord(paperclawExtension.projects) ? paperclawExtension.projects : {};
  const paperclawTasks = isPlainRecord(paperclawExtension.tasks) ? paperclawExtension.tasks : {};
  const paperclawRoutines = isPlainRecord(paperclawExtension.routines) ? paperclawExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(paperclawCompany.brandColor),
      logoPath: asString(paperclawCompany.logoPath) ?? asString(paperclawCompany.logo),
      attachmentMaxBytes:
        typeof paperclawCompany.attachmentMaxBytes === "number" && Number.isFinite(paperclawCompany.attachmentMaxBytes)
          ? Math.max(1, Math.floor(paperclawCompany.attachmentMaxBytes))
          : null,
      requireBoardApprovalForNewAgents:
        typeof paperclawCompany.requireBoardApprovalForNewAgents === "boolean"
          ? paperclawCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof paperclawCompany.feedbackDataSharingEnabled === "boolean"
          ? paperclawCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof paperclawCompany.feedbackDataSharingConsentAt === "string"
          ? paperclawCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(paperclawCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(paperclawCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: paperclawSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(paperclawAgents[slug]) ? paperclawAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(paperclawProjects[slug]) ? paperclawProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(paperclawTasks[slug]) ? paperclawTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(paperclawRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(paperclawRoutines[slug]) ? paperclawRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function prepareImportedAgentAdapter(
    ...
    const nextAdapterConfig = writePaperClawSkillSyncPreference(
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".paperclaw.yaml" ||
          relative === ".paperclaw.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const paperclawAgentsOut: Record<string, Record<string, unknown>> = {};
    const paperclawProjectsOut: Record<string, Record<string, unknown>> = {};
    const paperclawTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const paperclawRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readPaperClawSkillSyncPreference(
        ...
        paperclawAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      paperclawProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      paperclawTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      paperclawRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const paperclawExtensionPath = ".paperclaw.yaml";
    const paperclawAgents = Object.fromEntries(
      Object.entries(paperclawAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const paperclawProjects = Object.fromEntries(
      Object.entries(paperclawProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const paperclawTasks = Object.fromEntries(
      Object.entries(paperclawTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const paperclawRoutines = Object.fromEntries(
      Object.entries(paperclawRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[paperclawExtensionPath] = buildYamlFile(
      {
        schema: "paperclaw/v1",
        ...
        agents: Object.keys(paperclawAgents).length > 0 ? paperclawAgents : undefined,
        projects: Object.keys(paperclawProjects).length > 0 ? paperclawProjects : undefined,
        tasks: Object.keys(paperclawTasks).length > 0 ? paperclawTasks : undefined,
        routines: Object.keys(paperclawRoutines).length > 0 ? paperclawRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, paperclawExtensionPath);
