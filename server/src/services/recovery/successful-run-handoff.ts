...
export function buildSuccessfulRunHandoffInstruction(input: {
  ...
  return [
    `Your previous run on ${issueLabel} succeeded, but the issue is still in \`in_progress\` and PaperClaw cannot identify a valid issue disposition.`,
    "",
    "Resolve the missing disposition before creating or revising any new artifacts. Choose **exactly one** outcome and perform the matching PaperClaw action:",
    "",
...
export function decideSuccessfulRunHandoff(input: {
  ...
  if (input.hasOpenChildIssuePath) return { kind: "skip", reason: "open child issue path owns delegated follow-up" };
