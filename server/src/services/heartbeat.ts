...
import { extractSkillMentionIds } from "@kesarcloud/shared";
...
const DEFERRED_WAKE_CONTEXT_KEY = "_paperclawWakeContext";
const WAKE_COMMENT_IDS_KEY = "wakeCommentIds";
const PAPERCLAW_WAKE_PAYLOAD_KEY = "paperclawWake";
const PAPERCLAW_HARNESS_CHECKOUT_KEY = "paperclawHarnessCheckedOut";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const REPO_ONLY_CWD_SENTINEL = "/__paperclaw_repo_only__";
...
export function applyRunScopedMentionedSkillKeys(
  ...
  const existingPreference = readPaperClawSkillSyncPreference(config);
  return writePaperClawSkillSyncPreference(config, [
...
export function compactRunLogChunk(chunk: string, maxChars = MAX_PERSISTED_LOG_CHUNK_CHARS) {
  ...
  const marker = `\n[paperclaw truncated run log chunk: omitted ${omittedChars} chars]\n`;
...
async function buildPaperClawWakePayload(input: {
  db: Db;
  companyId: string;
  contextSnapshot: Record<string, unknown>;
  continuationSummary?:
  ...
  return {
    ...
    livenessContinuation: readNonEmptyString(input.contextSnapshot.livenessContinuationState) ||
      ...
      : null,
    checkedOutByHarness: input.contextSnapshot[PAPERCLAW_HARNESS_CHECKOUT_KEY] === true,
...
export function buildPaperClawTaskMarkdown(input: {
  issue: {
    id: string;
    identifier: string | null;
    title: string;
...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
  ...
  function monitorRecoveryComment(input: {
    ...
    return [
      `PaperClaw cleared the scheduled external-service monitor for ${label} because ${reason}.`,
      "",
          ...
          .where(
            and(
              ...
              sql`(
                ...
                or ${agentWakeupRequests.payload} -> '_paperclawWakeContext' ->> 'issueId' = ${issue.id}
                or ${agentWakeupRequests.payload} -> '_paperclawWakeContext' ->> 'taskId' = ${issue.id}
              )`,
            ),
          )
          .limit(1)
  ...
  async function cancelQueuedRunForBlockedDependencies(
    ...
    const now = new Date();
    const reason =
      "Cancelled because issue dependencies are still blocked; PaperClaw will wake the assignee when blockers resolve";
  ...
  async function executeRun(runId: string) {
    ...
    if (continuationSummary) {
      context.paperclawContinuationSummary = {
        key: continuationSummary.key,
        title: continuationSummary.title,
      ...
      delete context.paperclawContinuationSummary;
    }
    const paperclawWakePayload = await buildPaperClawWakePayload({
      db,
      companyId: agent.companyId,
      contextSnapshot: context,
      continuationSummary,
    ...
    if (paperclawWakePayload) {
      context[PAPERCLAW_WAKE_PAYLOAD_KEY] = paperclawWakePayload;
    } else {
      delete context[PAPERCLAW_WAKE_PAYLOAD_KEY];
    }
    const taskMarkdown = buildPaperClawTaskMarkdown({
      issue: issueRef
        ? {
            id: issueRef.id,
            identifier: issueRef.identifier,
    ...
    if (issueRef) {
      ...
      delete context.paperclawIssue;
    }
    if (wakeCommentContext) {
      context.paperclawWakeComment = wakeCommentContext;
    } else {
      delete context.paperclawWakeComment;
    }
    if (taskMarkdown) {
      context.paperclawTaskMarkdown = taskMarkdown;
    } else {
      delete context.paperclawTaskMarkdown;
    }
    ...
    let runtimeConfig = {
      ...
      paperclawRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.paperclawWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.paperclawRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.paperclawRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.paperclawSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.paperclawSessionRotationReason = sessionCompaction.reason;
      context.paperclawPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.paperclawSessionHandoffMarkdown;
      delete context.paperclawSessionRotationReason;
      delete context.paperclawPreviousSessionId;
    }
    ...
    try {
      ...
      if (runScopedMentionedSkillKeys.length > 0) {
        await onLog(
          "stdout",
          `[paperclaw] Enabled run-scoped skills from issue mentions: ${runScopedMentionedSkillKeys.join(", ")}\n`,
        );
      }
      ...
      if (runtimeServices.length > 0) {
        context.paperclawRuntimeServices = runtimeServices;
        context.paperclawRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[paperclaw] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.paperclawRuntimeServices = combinedRuntimeServices;
        context.paperclawRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[paperclaw] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
      ...
      if (finalizedRun) {
        ...
        if (issueId && outcome === "succeeded") {
          try {
            ...
            await onLog(
              "stderr",
              `[paperclaw] Failed to post run summary comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
  ...
  function buildImmediateExecutionPathRecoveryComment(input: {
    ...
    if (input.status === "todo") {
      return (
        "PaperClaw automatically retried dispatch for this assigned `todo` issue during terminal run recovery, " +
    ...
    return (
      "PaperClaw automatically retried continuation for this assigned `in_progress` issue during terminal run " +
