...
import { companies, companySkills } from "@kesarcloud/db";
import { readPaperClawSkillSyncPreference } from "@kesarcloud/adapter-utils/server-utils";
import type { PaperClawSkillEntry } from "@kesarcloud/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@kesarcloud/shared";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("karanbavari/paperclaw/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolvePaperClawInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "paperclaw_bundled") {
    return {
      editable: false,
      editableReason: "Bundled PaperClaw skills are read-only.",
      sourceLabel: "PaperClaw bundled",
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "PaperClaw workspace",
        sourceBadge: "paperclaw",
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "paperclaw_bundled";
      out.push({
        key: skill.key,
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "paperclaw_bundled"
        && incomingKind === "github"
        && incomingOwner === "paperclaw"
        && incomingRepo === "paperclaw"
      ) {
        out.push(existing);
        continue;
      }
