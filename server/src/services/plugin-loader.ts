 ...
 *    the `paperclaw-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published PaperClaw plugins.
 * Packages matching this pattern are considered PaperClaw plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "paperclaw-plugin-";
...
export type PluginSource =
  ...
  | "npm"               // npm packages matching paperclaw-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: PaperClawPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the paperclaw-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "paperclaw-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: PaperClawPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover PaperClaw plugins installed as npm packages in the current
   * Node.js environment matching the "paperclaw-plugin-*" naming convention.
   *
   ...
   * the "paperclawPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a PaperClaw plugin.
   * Throws if the package is a PaperClaw plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<PaperClawPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: PaperClawPluginManifestV1;
    newManifest: PaperClawPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the PaperClaw plugin naming convention.
 * Accepts both the "paperclaw-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
 ...
 *
 * The spec defines a "paperclawPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const paperclawPlugin = pkgJson["paperclawPlugin"];
  if (
    paperclawPlugin !== null &&
    typeof paperclawPlugin === "object" &&
    !Array.isArray(paperclawPlugin)
  ) {
    const manifestRelPath = (paperclawPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: PaperClawPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumPaperClawVersion;
}
 ...
 *   packageName: "paperclaw-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: PaperClawPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as PaperClawPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a PaperClaw plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<PaperClawPluginManifestV1> {
    let raw: unknown;
  ...
  async function loadManifestFromPackageRoot(
    packageRoot: string,
  ): Promise<PaperClawPluginManifestV1 | null> {
  ...
  async function refreshPluginManifestFromPackage(
    ...
    if (!manifest) {
      throw new Error(`Plugin package ${plugin.packageName} no longer exposes a PaperClaw manifest`);
    }
   ...
   * if the package is not a PaperClaw plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasPaperClawPlugin = "paperclawPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasPaperClawPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<PaperClawPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      const workerOptions: WorkerStartOptions = {
        ...
        autoRestart: true,
        env: {
          PAPERCLAW_DEPLOYMENT_MODE: instanceInfo.deploymentMode ?? "",
          PAPERCLAW_DEPLOYMENT_EXPOSURE: instanceInfo.deploymentExposure ?? "",
        },
      };
