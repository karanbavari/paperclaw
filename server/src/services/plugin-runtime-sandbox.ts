...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __paperclaw_exports: module.exports,
      __paperclaw_module: module,
      __paperclaw_require: requireInSandbox,
      __paperclaw_filename: realPath,
      __paperclaw_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__paperclaw_exports, __paperclaw_module, __paperclaw_require, __paperclaw_filename, __paperclaw_dirname)`;
