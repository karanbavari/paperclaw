  ...
  return {
    ...
    addSystemMessage: async (input: {
      ...
      const thread = await db
        .select({ ceoAgentId: directChatThreads.ceoAgentId })
        .from(directChatThreads)
        .where(and(
          eq(directChatThreads.companyId, input.companyId),
          eq(directChatThreads.id, input.threadId),
        ))
        .then((rows) => rows[0] ?? null);
      if (!thread) throw unprocessable("Direct Chat thread not found");
      const now = new Date();
      const [message] = await db.insert(directChatMessages).values({
        companyId: input.companyId,
      ...
      await db.update(directChatThreads).set({ updatedAt: now }).where(eq(directChatThreads.id, input.threadId));
      publishDirectChatMessageEvent(input.companyId, "direct_chat.message.created", {
        threadId: input.threadId,
        messageId: message.id,
        ceoAgentId: thread.ceoAgentId,
      });
      return message;
    },
