...
<head>
    <!-- PAPERCLAW_RUNTIME_BRANDING_START -->
    <!-- PAPERCLAW_RUNTIME_BRANDING_END -->
    <!-- PAPERCLAW_FAVICON_START -->
    <link rel="icon" href="/favicon.ico" sizes="48x48" />
    <link rel="icon" href="/favicon.svg" type="image/svg+xml" />
    <link rel="icon" type="image/png" sizes="32x32" href="/favicon-32x32.png" />
    <link rel="icon" type="image/png" sizes="16x16" href="/favicon-16x16.png" />
    <!-- PAPERCLAW_FAVICON_END -->
</head>`;

describe("ui branding", () => {
  it("detects worktree mode from PAPERCLAW_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ PAPERCLAW_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ PAPERCLAW_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ PAPERCLAW_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      PAPERCLAW_IN_WORKTREE: "true",
      PAPERCLAW_WORKTREE_NAME: "paperclaw-pr-432",
      PAPERCLAW_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("paperclaw-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="paperclaw-worktree-name"');
    expect(meta).toContain('content="paperclaw-pr-432"');
    expect(meta).toContain('name="paperclaw-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="paperclaw-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="paperclaw-worktree-name"');
  });
});
