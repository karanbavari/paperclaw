...
import { createAcpxLocalExecutor } from "@kesarcloud/adapter-acpx-local/server";
import type {
  AcpRuntime,
  AcpRuntimeEvent,
  AcpRuntimeHandle,
...
async function createRuntimeSkill(root: string, input: {
  ...
  const runtimeName = input.runtimeName ?? "paperclaw-test-skill";
...
describe("acpx_local execute", () => {
  it("streams ACPX session, status, text, and tool events before returning success", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-success-"));
    try {
  ...
  it("closes successful persistent runs by default while retaining session state", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-close-success-"));
    try {
      const runtime = new FakeRuntime({} as AcpRuntimeOptions);
      const execute = createAcpxLocalExecutor({
        createRuntime: () => runtime,
  ...
  it("applies requested Codex model, reasoning effort, and fast mode before starting the turn", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-codex-config-"));
    try {
      const runtime = new FakeRuntime({} as AcpRuntimeOptions);
      const execute = createAcpxLocalExecutor({
        createRuntime: () => runtime,
  ...
  it("logs a clear error when configured session options need unsupported runtime controls", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-missing-config-controls-"));
    try {
      const runtime = new FakeRuntime({} as AcpRuntimeOptions);
  ...
  it("reuses a compatible warm session and starts fresh when cwd changes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-reuse-"));
  ...
  it("closes duplicate warm handles from concurrent runs for the same session key", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-concurrent-"));
    try {
      const runtimes: FakeRuntime[] = [];
      ...
      expect(runtimes.flatMap((runtime) => runtime.closeInputs).filter((input) =>
        input.reason === "paperclaw duplicate warm handle cleanup"
      )).toHaveLength(1);
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  ...
  it("cleans configured warm handles after their idle window", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-warm-idle-"));
    vi.useFakeTimers();
    try {
      let clock = 0;
      const runtime = new FakeRuntime({} as AcpRuntimeOptions);
  ...
  it("retries with a fresh session when ACPX cannot resume the saved backend session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-resume-"));
    try {
      const runtime = new FakeRuntime({} as AcpRuntimeOptions);
  ...
  it("cancels and closes stale handles on timeout", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-timeout-"));
    try {
  ...
  it("returns structured auth errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-error-"));
    try {
      const runtime = new FakeRuntime({} as AcpRuntimeOptions);
  ...
  it("returns structured ACP protocol errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-protocol-"));
    try {
      const runtime = new FakeRuntime({} as AcpRuntimeOptions);
  ...
  it("materializes selected skills for ACPX Claude and passes public session metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-claude-skills-"));
    try {
      ...
      expect((meta?.commandNotes as string[]).join("\n")).toContain("Materialized 1 PaperClaw skill");
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("includes skill content in the ACPX Claude session fingerprint", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-claude-fingerprint-"));
    try {
  ...
  it("materializes selected skills into the effective ACPX Codex CODEX_HOME", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-codex-skills-"));
    try {
      const skill = await createRuntimeSkill(root, {});
  ...
  it("keeps ACPX custom skill selection tracked without runtime materialization", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-custom-skills-"));
    try {
      const skill = await createRuntimeSkill(root, {});
      let runtime: FakeRuntime | null = null;
      let meta: Record<string, unknown> | null = null;
