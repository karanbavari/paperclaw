...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "PaperClaw Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@kesarcloud.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("paperclaw-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `paperclaw-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "paperclaw-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "paperclaw-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "paperclaw-close-check",
      baseRef: "main",
      metadata: {
