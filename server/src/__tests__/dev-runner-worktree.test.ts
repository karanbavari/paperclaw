...
describe("dev-runner worktree env bootstrap", () => {
  it("detects linked git worktrees from .git files", () => {
    const root = createTempRoot("paperclaw-dev-runner-worktree-");
    fs.writeFileSync(path.join(root, ".git"), "gitdir: /tmp/paperclaw/.git/worktrees/feature\n", "utf8");

    expect(isLinkedGitWorktreeCheckout(root)).toBe(true);
  });

  it("loads repo-local PaperClaw env for initialized worktrees without overriding explicit env", () => {
    const root = createTempRoot("paperclaw-dev-runner-worktree-env-");
    ...
    expect(env.PAPERCLAW_HOME).toBe("/tmp/paperclaw-worktrees");
  ...
  it("reports uninitialized linked worktrees so dev runner can fail fast", () => {
    const root = createTempRoot("paperclaw-dev-runner-worktree-missing-");
    fs.writeFileSync(path.join(root, ".git"), "gitdir: /tmp/paperclaw/.git/worktrees/feature\n", "utf8");
