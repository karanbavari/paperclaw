...
describe("agent local JWT", () => {
  const secretEnv = "PAPERCLAW_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "PAPERCLAW_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "PAPERCLAW_AGENT_JWT_ISSUER";
  const audienceEnv = "PAPERCLAW_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when PAPERCLAW_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "paperclaw";
    process.env[audienceEnv] = "paperclaw-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
