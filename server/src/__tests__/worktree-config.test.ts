...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const paperclawDir = path.join(worktreeRoot, ".paperclaw");
    const configPath = path.join(paperclawDir, "config.json");
    const envPath = path.join(paperclawDir, ".env");
    const sharedRoot = path.join(tempRoot, ".paperclaw", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".paperclaw-worktrees");

    await fs.mkdir(paperclawDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# PaperClaw environment variables",
        "PAPERCLAW_IN_WORKTREE=true",
        "PAPERCLAW_WORKTREE_NAME=PAP-884-ai-commits-component",
        "PAPERCLAW_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.PAPERCLAW_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.PAPERCLAW_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`PAPERCLAW_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('PAPERCLAW_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`PAPERCLAW_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`PAPERCLAW_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('PAPERCLAW_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.PAPERCLAW_HOME).toBe(isolatedHome);
    expect(process.env.PAPERCLAW_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# PaperClaw environment variables",
        "PAPERCLAW_IN_WORKTREE=true",
        "PAPERCLAW_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.PAPERCLAW_IN_WORKTREE = "true";
    process.env.PAPERCLAW_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.PAPERCLAW_WORKTREES_DIR = isolatedHome;
    delete process.env.PAPERCLAW_HOME;
    delete process.env.PAPERCLAW_INSTANCE_ID;
    delete process.env.PAPERCLAW_CONFIG;
  ...
  it("does not persist transient runtime home overrides over repo-local worktree env", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-worktree-runtime-override-"));
    const isolatedHome = path.join(tempRoot, ".paperclaw-worktrees");
    ...
    await fs.writeFile(
      ...
      [
        "# PaperClaw environment variables",
        `PAPERCLAW_HOME=${JSON.stringify(isolatedHome)}`,
        `PAPERCLAW_INSTANCE_ID=${JSON.stringify(instanceId)}`,
        `PAPERCLAW_CONFIG=${JSON.stringify(configPath)}`,
        `PAPERCLAW_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`,
        'PAPERCLAW_IN_WORKTREE="true"',
        'PAPERCLAW_WORKTREE_NAME="PAP-989-multi-user-implementation-using-plan-from-pap-958"',
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.PAPERCLAW_IN_WORKTREE = "true";
    process.env.PAPERCLAW_WORKTREE_NAME = "PAP-989-multi-user-implementation-using-plan-from-pap-958";
    process.env.PAPERCLAW_HOME = transientHome;
    ...
    expect(repairedEnv).toContain(`PAPERCLAW_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).not.toContain(`PAPERCLAW_HOME=${JSON.stringify(transientHome)}`);
    expect(process.env.PAPERCLAW_HOME).toBe(isolatedHome);
  });

  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".paperclaw-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".paperclaw", "worktrees");
    ...
    const paperclawDir = path.join(currentWorktreeRoot, ".paperclaw");
    const configPath = path.join(paperclawDir, "config.json");
    const envPath = path.join(paperclawDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".paperclaw", "config.json");
  ...
  it("persists runtime-selected worktree ports back into explicit-port auth URLs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.PAPERCLAW_IN_WORKTREE = "true";
    process.env.PAPERCLAW_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.PAPERCLAW_HOME = isolatedHome;
    process.env.PAPERCLAW_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.PAPERCLAW_CONFIG = configPath;
  ...
  it("does not rewrite no-port public auth URLs when persisting runtime-selected ports", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-worktree-public-ports-"));
    ...
    await fs.writeFile(
      ...
      JSON.stringify(
        {
          ...buildLegacyConfig(instanceRoot, "https://paperclaw.example"),
          database: {
            mode: "embedded-postgres",
            embeddedPostgresDataDir: path.join(instanceRoot, "db"),
            embeddedPostgresPort: 54331,
    ...
    process.chdir(worktreeRoot);
    process.env.PAPERCLAW_IN_WORKTREE = "true";
    process.env.PAPERCLAW_WORKTREE_NAME = "PAP-125-public-base-url";
    process.env.PAPERCLAW_HOME = isolatedHome;
    process.env.PAPERCLAW_INSTANCE_ID = "pap-125-public-base-url";
    process.env.PAPERCLAW_CONFIG = configPath;
    ...
    expect(writtenConfig.server.port).toBe(3103);
    expect(writtenConfig.database.embeddedPostgresPort).toBe(54335);
    expect(writtenConfig.auth.publicBaseUrl).toBe("https://paperclaw.example");
  });
  ...
  it("does not rewrite the in-memory config when auth URL has no explicit port", () => {
    const { config, changed } = applyRuntimePortSelectionToConfig(
      buildLegacyConfig("/tmp/shared", "https://paperclaw.example"),
      {
        serverPort: 3104,
        databasePort: 54340,
        allowServerPortWrite: false,
    ...
    expect(config.server.port).toBe(3100);
    expect(config.database.embeddedPostgresPort).toBe(54340);
    expect(config.auth.publicBaseUrl).toBe("https://paperclaw.example");
  });
});
