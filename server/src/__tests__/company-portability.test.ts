...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "paperclaw",
        name: "paperclaw",
        description: "PaperClaw coordination skill",
        markdown: "---\nname: paperclaw\ndescription: PaperClaw coordination skill\n---\n\n# PaperClaw\n",
        sourceType: "github",
        sourceLocator: "https://github.com/karanbavari/paperclaw/tree/master/skills/paperclaw",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/paperclaw/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "paperclaw",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/paperclaw/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "paperclaw",
  ...
  it("exports referenced skills as stubs by default with sanitized PaperClaw extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "PaperClaw"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${paperclawKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/karanbavari/paperclaw/paperclaw/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/karanbavari/paperclaw/paperclaw/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/karanbavari/paperclaw/paperclaw/references/api.md"]).toBeUndefined();
    ...
    expect(extension).not.toContain("paperclawSkillSync");
  ...
  it("exports default sidebar order into the PaperClaw extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/karanbavari/paperclaw/paperclaw/SKILL.md"])).toContain("# PaperClaw");
    expect(asTextFile(exported.files["skills/karanbavari/paperclaw/paperclaw/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/karanbavari/paperclaw/paperclaw/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/karanbavari/paperclaw/paperclaw/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/karanbavari/paperclaw/paperclaw/SKILL.md"]).toBeDefined();
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-paperclaw",
        companyId: "company-1",
        key: "karanbavari/paperclaw/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/karanbavari/paperclaw/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "paperclaw_bundled",
          owner: "paperclaw",
          repo: "paperclaw",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/karanbavari/paperclaw/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/karanbavari/paperclaw/release-changelog/SKILL.md"])).toContain("karanbavari/paperclaw/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
    ...
    expect(extension).toContain('repoUrl: "https://github.com/karanbavari/paperclaw.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/paperclaw");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported PaperClaw",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/karanbavari/paperclaw.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".paperclaw.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/karanbavari/paperclaw.git"');
    expect(extension).toContain('projectWorkspaceKey: "paperclaw"');
  ...
  it("exports routines as recurring task packages with PaperClaw routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "paperclaw-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported PaperClaw" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported PaperClaw"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .paperclaw.yaml", async () => {
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported PaperClaw");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".paperclaw.yaml"] = `${exported.files[".paperclaw.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
