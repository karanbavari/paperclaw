...
export function directChatRoutes(
  ...
  async function assertBoardOrCeo(req: Request, companyId: string) {
    ...
    if (req.actor.type === "agent") {
      const ceo = await directChat.resolveCeoAgent(companyId);
      if (ceo.id === req.actor.agentId) return;
    }
    throw forbidden("Only Board users or the company CEO can access Direct Chat");
  }
