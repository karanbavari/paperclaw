...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@kesarcloud/db";
...
export function agentRoutes(
   ...
   *   adapter probes the PaperClaw host (legacy behavior).
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writePaperClawSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readPaperClawSkillSyncPreference(
