...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole, PermissionKey } from "@kesarcloud/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "paperclaw" &&
    normalized !== "paperclaw-create-agent" &&
    normalized !== "paperclaw-create-plugin" &&
    normalized !== "paperclaw-converting-plans-to-tasks" &&
...
/** Resolve the PaperClaw repo skills directory (built-in / managed skills). */
function resolvePaperClawSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const paperclawSkillsDir = resolvePaperClawSkillsDir();

  // Build set of PaperClaw-managed skill names
  const paperclawSkillNames = new Set<string>();
  if (paperclawSkillsDir) {
    try {
      for (const entry of fs.readdirSync(paperclawSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) paperclawSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isPaperClawManaged: paperclawSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.paperclawApiUrl)) {
    const legacyPaperClawApiUrl = nonEmptyTrimmedString(input.paperclawApiUrl);
    if (legacyPaperClawApiUrl) merged.paperclawApiUrl = legacyPaperClawApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    paperclawApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.paperclawApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawPaperClawApiUrl =
    typeof defaults.paperclawApiUrl === "string"
      ? defaults.paperclawApiUrl.trim()
      : "";
  if (rawPaperClawApiUrl) {
    try {
      const parsedPaperClawApiUrl = new URL(rawPaperClawApiUrl);
      if (
        parsedPaperClawApiUrl.protocol !== "http:" &&
        parsedPaperClawApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_paperclaw_api_url_protocol",
          level: "warn",
          message: `paperclawApiUrl must use http:// or https:// (got ${parsedPaperClawApiUrl.protocol}).`
        });
      } else {
        normalized.paperclawApiUrl = parsedPaperClawApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_paperclaw_api_url_configured",
          level: "info",
          message: `paperclawApiUrl set to ${parsedPaperClawApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_paperclaw_api_url_invalid",
        level: "warn",
        message: `Invalid paperclawApiUrl: ${rawPaperClawApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your PaperClaw host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "PaperClaw is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm paperclaw allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = `/api/invites/${token}/skills/paperclaw`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/paperclaw-claimed-api-key.json and load PAPERCLAW_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: paperclawApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the PaperClaw hostname is reachable and allowed via `pnpm paperclaw allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this PaperClaw API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/paperclaw/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # PaperClaw OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          paperclawApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, PaperClaw generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "paperclawApiUrl": "https://paperclaw-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in PaperClaw before key claim is allowed.
    ...
    - ~/.openclaw/workspace/paperclaw-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/paperclaw-claimed-api-key.json

    And set the PAPERCLAW_API_KEY and PAPERCLAW_API_URL in your environment variables as specified here:
    ...
    Then set PAPERCLAW_API_KEY and PAPERCLAW_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install PaperClaw skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your PAPERCLAW_API_URL to the top of your skill and note the path to your PAPERCLAW_API_URL
    ...
    ${
      ...
      "Ensure PaperClaw is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested PaperClaw base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.paperclawApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm paperclaw allowed-hostname <host>
      - then restart PaperClaw and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@kesarcloud.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "paperclaw", path: "/api/skills/paperclaw" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "paperclaw-create-agent",
          path: "/api/skills/paperclaw-create-agent"
        },
        {
          name: "paperclaw-converting-plans-to-tasks",
          path: "/api/skills/paperclaw-converting-plans-to-tasks"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "paperclaw",
          path: `/api/invites/${token}/skills/paperclaw`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "paperclaw") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              paperclawApiUrl: req.body.paperclawApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.paperclawApiUrl &&
          !persistedDefaults.paperclawApiUrl
        ) {
          missingPersistedFields.push("paperclawApiUrl");
        }
