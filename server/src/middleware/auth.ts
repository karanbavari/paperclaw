...
import { agentApiKeys, agents, authUsers, companies, companyMemberships, instanceUserRoles } from "@kesarcloud/db";
...
export function actorMiddleware(db: Db, opts: ActorMiddlewareOptions): RequestHandler {
  ...
  return async (req, _res, next) => {
    ...
    const runIdHeader = req.header("x-paperclaw-run-id");
...
async function resolveCloudTenantActor(db: Db, req: Request): Promise<Express.Request["actor"] | null> {
  const expectedToken = process.env.PAPERCLAW_CLOUD_TENANT_SERVER_TOKEN?.trim();
  if (!expectedToken) return null;

  const token = req.header("x-paperclaw-cloud-tenant-token")?.trim();
  if (!token || !constantTimeStringEqual(token, expectedToken)) return null;

  const userId = requiredCloudHeader(req, "x-paperclaw-cloud-user-id");
  const userEmail = requiredCloudHeader(req, "x-paperclaw-cloud-user-email").toLowerCase();
  const stackId = requiredCloudHeader(req, "x-paperclaw-cloud-stack-id");
  const stackRole = stackMembershipRole(req.header("x-paperclaw-cloud-stack-role"));
  const userName = req.header("x-paperclaw-cloud-user-name")?.trim() || userEmail;
  const paperclawCompanyId = req.header("x-paperclaw-cloud-paperclaw-company-id")?.trim();
  const companyId = cloudTenantCompanyId(stackId);
  const companyName = paperclawCompanyId || `${stackId} PaperClaw`;
  const now = new Date();
  ...
  await db
    ...
    .values({
      id: companyId,
      name: companyName,
      description: `Provisioned by PaperClaw Cloud for stack ${stackId}.`,
      status: "active",
...
function cloudTenantCompanyId(stackId: string): string {
  const bytes = createHash("sha256").update(`paperclaw-cloud-tenant-company:${stackId}`).digest();
