import type { PaperClawMcpConfig } from "./config.js";

export class PaperClawApiError extends Error {
...
export class PaperClawApiClient {
  constructor(private readonly config: PaperClawMcpConfig) {}
  ...
  resolveCompanyId(companyId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("companyId is required because PAPERCLAW_COMPANY_ID is not set");
    }
    return resolved;
  }
  ...
  resolveAgentId(agentId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("agentId is required because PAPERCLAW_AGENT_ID is not set");
    }
    return resolved;
  }
  ...
  async requestJson<T>(method: string, path: string, options: JsonRequestOptions = {}): Promise<T> {
    ...
    if ((options.includeRunId ?? isWriteMethod(method)) && this.config.runId) {
      headers["X-PaperClaw-Run-Id"] = this.config.runId;
    }
