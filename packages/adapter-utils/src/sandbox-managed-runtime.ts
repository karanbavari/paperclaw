...
export async function prepareSandboxManagedRuntime(input: {
  ...
  await withTempDir("paperclaw-sandbox-sync-", async (tempDir) => {
    ...
    const preservedNames = new Set([".paperclaw-runtime", ...(input.preserveAbsentOnRestore ?? [])]);
  ...
  return {
    ...
    restoreWorkspace: async () => {
      await withTempDir("paperclaw-sandbox-restore-", async (tempDir) => {
        ...
        await mirrorDirectory(extractedDir, input.workspaceLocalDir, {
          preserveAbsent: [".paperclaw-runtime", ...(input.preserveAbsentOnRestore ?? [])],
        });
      });
    },
  };
