...
export interface AdapterExecutionTargetPaperClawBridgeHandle {
  env: Record<string, string>;
  stop(): Promise<void>;
}
...
function resolveDefaultPaperClawApiUrl(): string {
  const runtimeHost = resolveHostForUrl(
    process.env.PAPERCLAW_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  // 3100 matches the default PaperClaw dev server port when the runtime does not provide one.
...
export function adapterExecutionTargetUsesPaperClawBridge(
  target: AdapterExecutionTarget | null | undefined,
): boolean {
  return target?.kind === "remote";
}
  ...
  if (detectCommand) {
    ...
    if (!recheck.timedOut && recheck.exitCode === 0) {
      if (input.onLog) {
        ...
        await input.onLog(
          "stderr",
          `[paperclaw] Install command ${reason} (${installCommand}) but ${detectCommand} is on PATH; continuing.\n`,
        );
      }
      return;
    }
 ...
 * the directory state inside the environment, not on the PaperClaw host.
 *
...
export async function startAdapterExecutionTargetPaperClawBridge(input: {
  runId: string;
  target: AdapterExecutionTarget | null | undefined;
...
}): Promise<AdapterExecutionTargetPaperClawBridgeHandle | null> {
  if (!adapterExecutionTargetUsesPaperClawBridge(input.target)) {
    return null;
  }
  ...
  if (hostApiToken.length === 0) {
    throw new Error("Sandbox bridge mode requires a host-side PaperClaw API token.");
  }
  ...
  const bridgeRuntimeDir = path.posix.join(runtimeRootDir, "paperclaw-bridge");
  ...
  const hostApiUrl =
    ...
    process.env.PAPERCLAW_RUNTIME_API_URL?.trim() ||
    process.env.PAPERCLAW_API_URL?.trim() ||
    resolveDefaultPaperClawApiUrl();
  const shellCommand = adapterExecutionTargetShellCommand(target);
  const runner = adapterExecutionTargetCommandRunner(target);

  await onLog(
    "stdout",
    `[paperclaw] Starting sandbox callback bridge for ${input.adapterKey} in ${bridgeRuntimeDir}.\n`,
  );
  ...
  try {
    ...
    // PAPERCLAW_BRIDGE_DEBUG opts into verbose stdout logs of every bridge
    ...
    worker = await startSandboxCallbackBridgeWorker({
      ...
      handleRequest: async (request) => {
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[paperclaw] Bridge proxy ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        ...
        headers.set("x-paperclaw-run-id", input.runId);
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[paperclaw] Bridge proxy response ${response.status} for ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        return {
