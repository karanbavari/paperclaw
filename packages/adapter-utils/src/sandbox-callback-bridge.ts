...
const SANDBOX_CALLBACK_BRIDGE_ENTRYPOINT = "paperclaw-bridge-server.mjs";
...
// reverse bridge. Keep this in sync with the PaperClaw skill in
// `skills/paperclaw/SKILL.md` and `references/api-reference.md`.
...
export async function createSandboxCallbackBridgeAsset(): Promise<SandboxCallbackBridgeAsset> {
  const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-bridge-asset-"));
...
export async function startSandboxCallbackBridgeWorker(input: {
  ...
  const processRequestFile = async (fileName: string) => {
    ...
    try {
      ...
      console.warn(
        `[paperclaw] sandbox callback bridge handler failed for ${request.id}: ${error instanceof Error ? error.message : String(error)}`,
      );
      await writeBridgeResponse(input.client, requestPath, responsePath, {
        id: request.id,
        status: 502,
  ...
  const failPendingRequests = async (message: string) => {
    ...
    for (const fileName of fileNames) {
      ...
      try {
        ...
        console.warn(
          `[paperclaw] sandbox callback bridge failed to abort pending request ${requestId}: ${error instanceof Error ? error.message : String(error)}`,
        );
      } finally {
...
export async function syncSandboxCallbackBridgeEntrypoint(input: {
  ...
  const remoteUploadPath = `${remoteEntrypoint}.paperclaw-upload.b64`;
  const remoteLockDir = path.posix.join(input.assetRemoteDir, ".paperclaw-bridge-upload.lock");
...
const responseTimeoutMs = Number(process.env.PAPERCLAW_BRIDGE_RESPONSE_TIMEOUT_MS || "30000");
