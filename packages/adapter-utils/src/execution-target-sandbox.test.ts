...
describe("sandbox adapter execution targets", () => {
  ...
  it("treats SSH targets as bridge-only", () => {
    ...
    expect(adapterExecutionTargetUsesPaperClawBridge(target)).toBe(true);
    expect(adapterExecutionTargetSessionIdentity(target)).toEqual({
      transport: "ssh",
      host: "ssh.example.test",
      port: 22,
  ...
  it("starts a localhost PaperClaw bridge for sandbox targets in bridge mode", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclaw-execution-target-bridge-"));
    ...
    const apiServer = createServer((req, res) => {
      requests.push({
        ...
        runId: typeof req.headers["x-paperclaw-run-id"] === "string" ? req.headers["x-paperclaw-run-id"] : null,
      });
    ...
    const bridge = await startAdapterExecutionTargetPaperClawBridge({
      runId: "run-bridge",
      target,
      runtimeRootDir,
      adapterKey: "codex",
  ...
  it("fails oversized host responses with a 502 before returning them to the sandbox client", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclaw-execution-target-bridge-limit-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".paperclaw-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
