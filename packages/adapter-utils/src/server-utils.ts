...
const PAPERCLAW_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];
const MATERIALIZED_SKILL_SENTINEL = ".paperclaw-materialized-skill.json";
const MATERIALIZED_SKILL_LOCK_OWNER = "owner.json";
const MATERIALIZED_SKILL_LOCK_STALE_MS = 30_000;

export const DEFAULT_PAPERCLAW_AGENT_PROMPT_TEMPLATE = [
  "You are agent {{agent.id}} ({{agent.name}}). Continue your PaperClaw work.",
  "",
  "Execution contract:",
  ...
  "- Keep delegation one level deep by default. Child assignees must do the work themselves and must not create more child issues unless the board/user or parent issue explicitly authorizes it.",
  "- When delegating known follow-up work from a parent issue, create or reuse a child issue with status todo, the right assigneeAgentId, and blockParentUntilDone true so the parent has an explicit delegated follow-up disposition.",
  "- If woken by a human comment on a dependency-blocked issue, respond or triage the comment without treating the blocked deliverable work as unblocked.",
  "- Create a small bounded set of child issues only when ownership and scope are clear; use issue-thread interactions when the board/user must choose suggested tasks, answer structured questions, or confirm a proposal.",
...
export interface PaperClawSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
export interface MaterializedPaperClawSkillCopyResult {
  copiedFiles: number;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: PaperClawSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "paperclaw_required",
      originLabel: "Required by PaperClaw",
      readOnly: false,
    };
  }
  return {
...
type PaperClawWakeExecutionPrincipal = {
  type: "agent" | "user" | null;
  agentId: string | null;
  userId: string | null;
};

type PaperClawWakeExecutionStage = {
  wakeRole: "reviewer" | "approver" | "executor" | null;
  stageId: string | null;
  stageType: string | null;
  currentParticipant: PaperClawWakeExecutionPrincipal | null;
  returnAssignee: PaperClawWakeExecutionPrincipal | null;
  reviewRequest: {
    instructions: string;
  } | null;
...
type PaperClawWakeContinuationSummary = {
  key: string | null;
  title: string | null;
  body: string;
  bodyTruncated: boolean;
...
type PaperClawWakeLivenessContinuation = {
  attempt: number | null;
...
type PaperClawWakeChildIssueSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type PaperClawWakeBlockerSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type PaperClawWakeTreeHoldSummary = {
  holdId: string | null;
...
type PaperClawWakePayload = {
  reason: string | null;
  issue: PaperClawWakeIssue | null;
  checkedOutByHarness: boolean;
  dependencyBlockedInteraction: boolean;
  treeHoldInteraction: boolean;
  activeTreeHold: PaperClawWakeTreeHoldSummary | null;
  unresolvedBlockerIssueIds: string[];
  unresolvedBlockerSummaries: PaperClawWakeBlockerSummary[];
  executionStage: PaperClawWakeExecutionStage | null;
  continuationSummary: PaperClawWakeContinuationSummary | null;
  livenessContinuation: PaperClawWakeLivenessContinuation | null;
  childIssueSummaries: PaperClawWakeChildIssueSummary[];
  childIssueSummaryTruncated: boolean;
  commentIds: string[];
  latestCommentId: string | null;
  comments: PaperClawWakeComment[];
...
function normalizePaperClawWakeIssue(value: unknown): PaperClawWakeIssue | null {
...
function normalizePaperClawWakeComment(value: unknown): PaperClawWakeComment | null {
...
function normalizePaperClawWakeContinuationSummary(value: unknown): PaperClawWakeContinuationSummary | null {
...
function normalizePaperClawWakeLivenessContinuation(value: unknown): PaperClawWakeLivenessContinuation | null {
...
function normalizePaperClawWakeChildIssueSummary(value: unknown): PaperClawWakeChildIssueSummary | null {
...
function normalizePaperClawWakeBlockerSummary(value: unknown): PaperClawWakeBlockerSummary | null {
...
function normalizePaperClawWakeTreeHoldSummary(value: unknown): PaperClawWakeTreeHoldSummary | null {
...
function normalizePaperClawWakeExecutionPrincipal(value: unknown): PaperClawWakeExecutionPrincipal | null {
...
function normalizePaperClawWakeExecutionStage(value: unknown): PaperClawWakeExecutionStage | null {
  ...
  const allowedActions = Array.isArray(stage.allowedActions)
    ...
    : [];
  const currentParticipant = normalizePaperClawWakeExecutionPrincipal(stage.currentParticipant);
  const returnAssignee = normalizePaperClawWakeExecutionPrincipal(stage.returnAssignee);
...
export function normalizePaperClawWakePayload(value: unknown): PaperClawWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizePaperClawWakeComment(entry))
        .filter((entry): entry is PaperClawWakeComment => Boolean(entry))
    : [];
  ...
  const commentIds = Array.isArray(payload.commentIds)
    ? payload.commentIds
        ...
        .map((entry) => entry.trim())
    : [];
  const executionStage = normalizePaperClawWakeExecutionStage(payload.executionStage);
  const continuationSummary = normalizePaperClawWakeContinuationSummary(payload.continuationSummary);
  const livenessContinuation = normalizePaperClawWakeLivenessContinuation(payload.livenessContinuation);
  const childIssueSummaries = Array.isArray(payload.childIssueSummaries)
    ? payload.childIssueSummaries
        .map((entry) => normalizePaperClawWakeChildIssueSummary(entry))
        .filter((entry): entry is PaperClawWakeChildIssueSummary => Boolean(entry))
    : [];
  ...
  const unresolvedBlockerSummaries = Array.isArray(payload.unresolvedBlockerSummaries)
    ? payload.unresolvedBlockerSummaries
        .map((entry) => normalizePaperClawWakeBlockerSummary(entry))
        .filter((entry): entry is PaperClawWakeBlockerSummary => Boolean(entry))
    : [];

  const activeTreeHold = normalizePaperClawWakeTreeHoldSummary(payload.activeTreeHold);
  if (comments.length === 0 && commentIds.length === 0 && childIssueSummaries.length === 0 && unresolvedBlockerIssueIds.length === 0 && unresolvedBlockerSummaries.length === 0 && !activeTreeHold && !executionStage && !continuationSummary && !livenessContinuation && !normalizePaperClawWakeIssue(payload.issue)) {
    return null;
  }

  return {
    reason: asString(payload.reason, "").trim() || null,
    issue: normalizePaperClawWakeIssue(payload.issue),
...
export function stringifyPaperClawWakePayload(value: unknown): string | null {
  const normalized = normalizePaperClawWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function renderPaperClawWakePrompt(
  value: unknown,
  ...
  const principalLabel = (principal: PaperClawWakeExecutionPrincipal | null) => {
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "PAPERCLAW_RESOLVED_COMMAND"] = redactCommandTextForLogs(resolvedCommand);
  }

  return redactEnvForLogs(merged);
}

export function buildPaperClawEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    PAPERCLAW_AGENT_ID: agent.id,
    PAPERCLAW_COMPANY_ID: agent.companyId,
  };
  ...
  const apiUrl =
    process.env.PAPERCLAW_RUNTIME_API_URL ??
    process.env.PAPERCLAW_API_URL ??
    `http://${runtimeHost}:${runtimePort}`;
  vars.PAPERCLAW_API_URL = apiUrl;
  return vars;
}

export function applyPaperClawWorkspaceEnv(
  env: Record<string, string>,
  input: {
  ...
  const mappings = [
    ["PAPERCLAW_WORKSPACE_CWD", input.workspaceCwd],
    ["PAPERCLAW_WORKSPACE_SOURCE", input.workspaceSource],
    ["PAPERCLAW_WORKSPACE_STRATEGY", input.workspaceStrategy],
    ["PAPERCLAW_WORKSPACE_ID", input.workspaceId],
    ["PAPERCLAW_WORKSPACE_REPO_URL", input.workspaceRepoUrl],
    ["PAPERCLAW_WORKSPACE_REPO_REF", input.workspaceRepoRef],
    ["PAPERCLAW_WORKSPACE_BRANCH", input.workspaceBranch],
    ["PAPERCLAW_WORKSPACE_WORKTREE_PATH", input.workspaceWorktreePath],
...
export function shapePaperClawWorkspaceEnvForExecution(input: {
  workspaceCwd?: string | null;
  workspaceWorktreePath?: string | null;
  ...
  if (executionCwd === null) {
    ...
    console.warn(
      "[paperclaw] shapePaperClawWorkspaceEnvForExecution called with executionCwd=null on a remote target; " +
...
export function sanitizeInheritedPaperClawEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...baseEnv };
  for (const key of Object.keys(env)) {
    if (!key.startsWith("PAPERCLAW_")) continue;
    if (key === "PAPERCLAW_RUNTIME_API_URL") continue;
    if (key === "PAPERCLAW_LISTEN_HOST") continue;
    if (key === "PAPERCLAW_LISTEN_PORT") continue;
    delete env[key];
  }
  return env;
}
...
export async function resolvePaperClawSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...PAPERCLAW_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listPaperClawSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<PaperClawSkillEntry[]> {
  const root = await resolvePaperClawSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return Promise.all(dirs.map(async (entry) => {
      ...
      return {
        key: `karanbavari/paperclaw/${entry.name}`,
        runtimeName: entry.name,
        source: skillDir,
        required,
        requiredReason: required
          ? "Bundled PaperClaw skills are always available for local adapters."
          : null,
      };
    }));
  } catch {
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the PaperClaw skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "PaperClaw cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredPaperClawRuntimeSkills(value: unknown): PaperClawSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: PaperClawSkillEntry[] = [];
...
export async function readPaperClawRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<PaperClawSkillEntry[]> {
  const configuredEntries = normalizeConfiguredPaperClawRuntimeSkills(config.paperclawRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listPaperClawSkillEntries(moduleDir, additionalCandidates);
}

export async function readPaperClawSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listPaperClawSkillEntries(moduleDir);
...
export function readPaperClawSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.paperclawSkillSync;
...
function canonicalizeDesiredPaperClawSkillReference(
  reference: string,
...
export function resolvePaperClawDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readPaperClawSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredPaperClawSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writePaperClawSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.paperclawSkillSync;
  const current =
  ...
  next.paperclawSkillSync = current;
  return next;
}

export async function ensurePaperClawSkillSymlink(
  source: string,
  target: string,
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`Timed out waiting for PaperClaw skill materialization lock at ${lockDir}`);
      }
...
export async function materializePaperClawSkillCopy(
  source: string,
  target: string,
): Promise<MaterializedPaperClawSkillCopyResult> {
  ...
  if (!rootStat.isDirectory()) {
    throw new Error("PaperClaw skills must be directories.");
  }

  const result: MaterializedPaperClawSkillCopyResult = {
    copiedFiles: 0,
    skippedSymlinks: [],
  };
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    const rawMerged: NodeJS.ProcessEnv = {
      ...sanitizeInheritedPaperClawEnv(process.env),
      ...opts.env,
    };

    // Strip Claude Code nesting-guard env vars so spawned `claude` processes
    // don't refuse to start with "cannot be launched inside another session".
    // These vars leak in when the PaperClaw server itself is started from
    // within a Claude Code session (e.g. `npx paperclaw run` in a terminal
