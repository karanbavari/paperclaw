...
describe("materializePaperClawSkillCopy", () => {
  it("refuses to materialize into an ancestor of the source", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-skill-copy-"));
    try {
      const source = path.join(root, "parent", "skill");
      await fs.mkdir(source, { recursive: true });
      await fs.writeFile(path.join(source, "SKILL.md"), "# skill\n", "utf8");

      await expect(materializePaperClawSkillCopy(source, path.join(root, "parent"))).rejects.toThrow(
        /ancestor/,
      );
  ...
  it("does not delete and recopy an unchanged materialized skill target", async () => {
    ...
    try {
      ...
      const first = await materializePaperClawSkillCopy(source, target);
      expect(first.copiedFiles).toBe(1);
      await fs.writeFile(path.join(target, "local-marker.txt"), "keep\n", "utf8");

      const second = await materializePaperClawSkillCopy(source, target);
  ...
  it("breaks stale materialization locks left by dead processes", async () => {
    ...
    try {
      ...
      await expect(materializePaperClawSkillCopy(source, target)).resolves.toMatchObject({ copiedFiles: 1 });
...
describe("renderPaperClawWakePrompt", () => {
  it("keeps the default local-agent prompt action-oriented", () => {
    ...
    expect(DEFAULT_PAPERCLAW_AGENT_PROMPT_TEMPLATE).toContain("Wait for acceptance before creating implementation subtasks");
  ...
  it("adds the execution contract to scoped wake prompts", () => {
    const prompt = renderPaperClawWakePrompt({
  ...
  it("includes continuation and child issue summaries in structured wake context", () => {
    ...
    expect(JSON.parse(stringifyPaperClawWakePayload(payload) ?? "{}")).toMatchObject({
      continuationSummary: {
    ...
    const prompt = renderPaperClawWakePrompt(payload);
  ...
  it("adds shared workspace env vars including AGENT_HOME", () => {
    const env = applyPaperClawWorkspaceEnv(
      {},
      {
        ...
        workspaceRepoUrl: "https://github.com/karanbavari/paperclaw.git",
    ...
    expect(env).toEqual({
      ...
      PAPERCLAW_WORKSPACE_REPO_URL: "https://github.com/karanbavari/paperclaw.git",
...
describe("shapePaperClawWorkspaceEnvForExecution", () => {
  it("rewrites workspace env paths for remote execution", () => {
    const shaped = shapePaperClawWorkspaceEnvForExecution({
      workspaceCwd: "/tmp/workspace",
      workspaceWorktreePath: "/tmp/worktree",
      workspaceHints: [
        {
