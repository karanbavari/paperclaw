...
async function createSshAuthArgs(
  ...
  if (config.strictHostKeyChecking) {
    if (config.knownHosts) {
      const knownHosts = await withTempFile("paperclaw-ssh-known-hosts-", config.knownHosts, 0o600);
  ...
  if (config.privateKey) {
    const privateKey = await withTempFile("paperclaw-ssh-key-", config.privateKey, 0o600);
...
async function importGitWorkspaceToSsh(input: {
  ...
  const bundleDir = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-ssh-bundle-"));
  const bundlePath = path.join(bundleDir, "workspace.bundle");
  const tempRef = "refs/paperclaw/ssh-sync/import";
  ...
  try {
    ...
    const remoteSetupScript = [
      "set -e",
      `mkdir -p ${shellQuote(path.posix.join(input.remoteDir, ".paperclaw-runtime"))}`,
      `tmp_bundle=$(mktemp ${shellQuote(path.posix.join(input.remoteDir, ".paperclaw-runtime", "import-XXXXXX.bundle"))})`,
      ...
      `git -C ${shellQuote(input.remoteDir)} clean -fdx -e .paperclaw-runtime >/dev/null`,
    ].join("\n");
...
async function exportGitWorkspaceFromSsh(input: {
  ...
  const bundleDir = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-ssh-bundle-"));
  const bundlePath = path.join(bundleDir, "workspace.bundle");
  const importedRef = "refs/paperclaw/ssh-sync/imported";

  try {
    const exportScript = [
      "set -e",
      `git -C ${shellQuote(input.remoteDir)} update-ref refs/paperclaw/ssh-sync/export HEAD`,
      `mkdir -p ${shellQuote(path.posix.join(input.remoteDir, ".paperclaw-runtime"))}`,
      `tmp_bundle=$(mktemp ${shellQuote(path.posix.join(input.remoteDir, ".paperclaw-runtime", "export-XXXXXX.bundle"))})`,
      'cleanup() { rm -f "$tmp_bundle"; git -C ' + shellQuote(input.remoteDir) + ' update-ref -d refs/paperclaw/ssh-sync/export >/dev/null 2>&1 || true; }',
      'trap cleanup EXIT',
      `git -C ${shellQuote(input.remoteDir)} bundle create "$tmp_bundle" refs/paperclaw/ssh-sync/export >/dev/null`,
      'cat "$tmp_bundle"',
    ].join("\n");
    ...
    await runLocalGit(input.localDir, ["fetch", "--force", bundlePath, `refs/paperclaw/ssh-sync/export:${importedRef}`], {
      timeout: 60_000,
      maxBuffer: 1024 * 1024,
    });
...
export async function syncDirectoryFromSsh(input: {
  ...
  const auth = await createSshAuthArgs(input.spec);
  const stagingDir = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-ssh-sync-back-"));
...
export async function prepareWorkspaceForSshExecution(input: {
  ...
  await clearRemoteDirectory({
    spec: input.spec,
    remoteDir,
    preserveEntries: [".paperclaw-runtime"],
  });
  await syncDirectoryToSsh({
    spec: input.spec,
    localDir: input.localDir,
