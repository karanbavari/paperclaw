...
function StatusPanel({ compact = false }: { compact?: boolean }) {
  ...
  if (status.loading) return <div style={mutedStyle}>Checking Stripe...</div>;
  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      ...
      {!compact ? (
        <>
          <div style={mutedStyle}>Gateway: {data?.apiBaseUrl ?? "https://api.stripe.com/v1"}</div>
...
function RecentCommands() {
  ...
  if (recent.loading) return <div style={mutedStyle}>Loading recent Stripe commands...</div>;
  if (recent.error) return <div style={mutedStyle}>Recent command audit is unavailable.</div>;
  const commands = recent.data?.commands ?? [];
  if (commands.length === 0) return <div style={mutedStyle}>No Stripe commands have been run yet.</div>;
...
function RecentWebhooks() {
  ...
  if (webhooks.length === 0) return <div style={mutedStyle}>No Stripe webhooks have been received yet.</div>;
    ...
    <div style={{ display: "grid", gap: 8 }}>
      {webhooks.map((event) => (
        <div key={event.eventId} style={{ borderTop: "1px solid var(--border, #d7d7d7)", paddingTop: 8 }}>
          ...
          <div style={mutedStyle}>{event.objectType} · {event.receivedAt}</div>
        </div>
      ))}
    </div>
  );
...
export function StripeDashboardWidget(_props: PluginWidgetProps) {
  return (
...
export function StripePage(_props: PluginPageProps) {
  return (
    <main style={{ display: "grid", gap: 16, padding: 24 }}>
      <header>
        <h1 style={{ margin: 0, fontSize: 22 }}>Stripe</h1>
        <p style={{ ...mutedStyle, marginTop: 6 }}>
          Merchant tools for PaymentIntents, refunds, Checkout Sessions, customers, products, prices, and webhooks.
        </p>
      </header>
...
export function StripeSettingsPage(_props: PluginSettingsPageProps) {
  return (
    <section style={{ display: "grid", gap: 16 }}>
      ...
      <div style={panelStyle}>
        ...
        <div style={mutedStyle}>
          Add a PaperClaw secret reference for the Stripe secret or restricted key. Keep test and live keys separated by plugin configuration.
        </div>
      </div>
      <div style={panelStyle}>
        <h2 style={{ margin: "0 0 8px", fontSize: 16 }}>Agent guardrails</h2>
        <div style={mutedStyle}>
          Mutating tools respect dry-run mode, operation allowlists, currency allowlists, amount limits, and optional idempotency keys.
        </div>
      </div>
    </section>
  );
