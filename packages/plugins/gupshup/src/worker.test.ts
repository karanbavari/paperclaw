...
describe("Gupshup communication plugin", () => {
  it("declares communication category and core tools", () => {
    expect(manifest.categories).toContain("communication");
    expect(manifest.tools?.map((tool) => tool.name)).toContain("gupshup.apiRequest");
    expect(definition.packageName).toBe("@kesarcloud/plugin-gupshup");
  });
  ...
  it("prepares mutating requests without calling external APIs in dry-run mode", async () => {
    ...
    const result = await harness.executeTool("gupshup.messageSend", {
      "message": {
            "to": "+15551234567",
            "text": "Example"
      }
