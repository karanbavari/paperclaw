...
describe("Sinch communication plugin", () => {
  it("declares communication category and core tools", () => {
    expect(manifest.categories).toContain("communication");
    expect(manifest.tools?.map((tool) => tool.name)).toContain("sinch.apiRequest");
    expect(definition.packageName).toBe("@kesarcloud/plugin-sinch");
  });
  ...
  it("prepares mutating requests without calling external APIs in dry-run mode", async () => {
    ...
    const result = await harness.executeTool("sinch.messageSend", {
      "projectId": "project-1",
      "message": {
            "to": "+15551234567",
            "text": "Example"
