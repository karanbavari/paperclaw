...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".paperclaw-sdk");
...
/**
 * Generate a complete PaperClaw plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A PaperClaw plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "paperclaw-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["paperclaw", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@kesarcloud/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@kesarcloud/plugin-sdk": sdkDependency,
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@kesarcloud/plugin-sdk/ui";
...
} from "@kesarcloud/plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@kesarcloud/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@kesarcloud/plugin-sdk\` and \`@kesarcloud/shared\` from a local PaperClaw checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.paperclaw-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-paperclaw-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <paperclaw-sdk-path>]");
    process.exit(1);
  }
