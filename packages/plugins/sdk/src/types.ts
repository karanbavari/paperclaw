/**
 * Core types for the PaperClaw plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@kesarcloud/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @kesarcloud/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of PaperClaw object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the PaperClaw UI. */
  title?: string;
...
export interface PluginLocalFoldersClient {
  ...
  declarations(): import("@kesarcloud/shared").PluginLocalFolderDeclaration[];
...
/**
 * `ctx.events` — subscribe to and emit PaperClaw domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core PaperClaw domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the PaperClaw secret provider
...
export interface PluginSecretsClient {
   ...
   * in the PaperClaw secret provider (e.g. `"MY_API_KEY"`).
   *
...
/**
 * `ctx.routines` — resolve and reconcile plugin-managed PaperClaw routines.
 *
 ...
 * ```
 *
 * @see usePluginStream in `@kesarcloud/plugin-sdk/ui`
 */
