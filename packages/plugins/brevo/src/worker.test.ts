...
describe("Brevo communication plugin", () => {
  it("declares communication category and core tools", () => {
    expect(manifest.categories).toContain("communication");
    expect(manifest.tools?.map((tool) => tool.name)).toContain("brevo.apiRequest");
    expect(definition.packageName).toBe("@kesarcloud/plugin-brevo");
  });
  ...
  it("prepares mutating requests without calling external APIs in dry-run mode", async () => {
    ...
    const result = await harness.executeTool("brevo.smtpEmailSend", {
      "message": {
            "to": "+15551234567",
            "text": "Example"
      }
