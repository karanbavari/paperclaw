...
import { legalActionKeys, legalDataKeys, type LegalDefinition } from "./shared.js";
...
export function createLegalUi(definition: LegalDefinition, pluginId: string) {
  ...
  function StatusPanel({ compact = false }: { compact?: boolean }) {
    ...
    const status = usePluginData<StatusData>(legalDataKeys.status, { companyId: host.companyId });
  ...
  function RecentCommands() {
    const host = useHostContext();
    const recent = usePluginData<RecentCommandsData>(legalDataKeys.recentCommands, { companyId: host.companyId });
  ...
  function SetupForm() {
    const host = useHostContext();
    const saveAccessToken = usePluginAction(legalActionKeys.saveAccessToken);
    const saveClientSecret = usePluginAction(legalActionKeys.saveClientSecret);
    const startOauth = usePluginAction(legalActionKeys.startOauth);
    const completeOauth = usePluginAction(legalActionKeys.completeOauth);
    const status = usePluginData<StatusData>(legalDataKeys.status, { companyId: host.companyId });
  ...
  function LegalDashboardWidget(_props: PluginWidgetProps) {
    return <section style={{ display: "grid", gap: 12 }}><StatusPanel compact /><RecentCommands /></section>;
  }

  function LegalPage(_props: PluginPageProps) {
    return (
      <main style={{ display: "grid", gap: 16, maxWidth: 920 }}>
        ...
        <section style={panelStyle}><h2 style={{ margin: "0 0 12px", fontSize: 18 }}>Available Tool Groups</h2><div style={{ display: "grid", gap: 8, fontSize: 14 }}><div>{definition.description}</div><div>Core tools include read/search/list plus dry-run protected write operations.</div><div>Keep dry run enabled while agents prepare legal workspace changes.</div><div>These tools operate legal records and documents; they do not provide legal advice.</div></div></section>
      </main>
    );
  }

  function LegalSettingsPage(_props: PluginSettingsPageProps) {
  ...
  return { LegalDashboardWidget, LegalPage, LegalSettingsPage };
}
