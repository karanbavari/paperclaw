...
import { legalActionKeys, legalDataKeys, type LegalDefinition, type LegalAuthMode, type LegalEndpoint } from "./shared.js";

export { legalActionKeys, legalDataKeys, type LegalDefinition, type LegalEndpoint } from "./shared.js";

export type LegalConfig = {
  authMode: LegalAuthMode;
...
export function dataKey(definition: LegalDefinition, suffix: string) {
...
function normalizeConfig(definition: LegalDefinition, raw: Record<string, unknown>): LegalConfig {
  return {
...
function validateConfig(definition: LegalDefinition, raw: Record<string, unknown>) {
  ...
  warnings.push(`${definition.displayName} tools automate legal operations data only; they do not provide legal advice.`);
  return { config, errors, warnings };
}

function isConnected(config: LegalConfig) {
...
function toolName(definition: LegalDefinition, key: string) {
  return `${definition.routePath}.${key}`;
}

function manifestParameters(endpoint: LegalEndpoint) {
...
export function createLegalManifest(definition: LegalDefinition): PaperClawPluginManifestV1 {
  return {
    id: definition.id,
    apiVersion: 1,
    ...
    author: "PaperClaw",
    categories: ["legal_law", "connector", "automation", "workspace"],
    capabilities: [
      "http.outbound",
      "secrets.read-ref",
      "secrets.write-ref",
    ...
    ui: {
      slots: [
        { type: "page", id: `${definition.routePath}-page`, displayName: definition.displayName, exportName: "LegalPage", routePath: definition.routePath, order: 70 },
        { type: "settingsPage", id: `${definition.routePath}-settings-page`, displayName: definition.displayName, exportName: "LegalSettingsPage", order: 70 },
        { type: "dashboardWidget", id: `${definition.routePath}-dashboard-widget`, displayName: definition.displayName, exportName: "LegalDashboardWidget", order: 70 },
      ],
...
function buildPlan(definition: LegalDefinition, name: string, params: Params, config: LegalConfig): LegalRequestPlan {
  ...
  if (name === rawTool) {
    ...
    return {
      ...
      method: method as LegalRequestPlan["method"],
      path,
...
async function tokenRequest(ctx: PluginContext, definition: LegalDefinition, config: LegalConfig, body: Record<string, string>): Promise<TokenResponse> {
...
async function resolveAccessToken(ctx: PluginContext, definition: LegalDefinition, config: LegalConfig, companyId: string) {
...
async function request(ctx: PluginContext, definition: LegalDefinition, config: LegalConfig, plan: LegalRequestPlan, companyId: string) {
...
async function audit(ctx: PluginContext, definition: LegalDefinition, runCtx: ToolRunContext, plan: LegalRequestPlan, result: { ok: boolean; dryRun: boolean }) {
...
async function executePlan(ctx: PluginContext, definition: LegalDefinition, runCtx: ToolRunContext, plan: LegalRequestPlan): Promise<ToolResult> {
...
export function createLegalPlugin(definition: LegalDefinition) {
  return definePlugin({
    async setup(ctx) {
      ...
      ctx.data.register(legalDataKeys.status, async (params) => {
      ...
      ctx.data.register(legalDataKeys.recentCommands, async (params) => {
        const companyId = stringParam(asObject(params), "companyId");
      ...
      ctx.actions.register(legalActionKeys.saveAccessToken, async (params) => ctx.secrets.upsert({
      ...
      ctx.actions.register(legalActionKeys.saveClientSecret, async (params) => ctx.secrets.upsert({
        companyId: requireString(asObject(params), "companyId"),
      ...
      ctx.actions.register(legalActionKeys.startOauth, async (params) => {
      ...
      ctx.actions.register(legalActionKeys.completeOauth, async (params) => {
      ...
      ctx.actions.register(legalActionKeys.disconnect, async (params) => {
        const companyId = stringParam(asObject(params), "companyId");
...
export function runLegalWorker(definition: LegalDefinition, importMetaUrl: string) {
  runWorker(createLegalPlugin(definition), importMetaUrl);
}
