...
import { developerActionKeys, developerDataKeys, type DeveloperDefinition } from "./shared.js";
...
export function createDeveloperUi(definition: DeveloperDefinition, pluginId: string) {
  ...
  function StatusPanel({ compact = false }: { compact?: boolean }) {
    ...
    const status = usePluginData<StatusData>(developerDataKeys.status, { companyId: host.companyId });
  ...
  function RecentCommands() {
    const host = useHostContext();
    const recent = usePluginData<RecentCommandsData>(developerDataKeys.recentCommands, { companyId: host.companyId });
  ...
  function SetupForm() {
    const host = useHostContext();
    const saveAccessToken = usePluginAction(developerActionKeys.saveAccessToken);
    const saveClientSecret = usePluginAction(developerActionKeys.saveClientSecret);
    const startOauth = usePluginAction(developerActionKeys.startOauth);
    const completeOauth = usePluginAction(developerActionKeys.completeOauth);
    const status = usePluginData<StatusData>(developerDataKeys.status, { companyId: host.companyId });
  ...
  function DeveloperDashboardWidget(_props: PluginWidgetProps) {
    return <section style={{ display: "grid", gap: 12 }}><StatusPanel compact /><RecentCommands /></section>;
  }

  function DeveloperPage(_props: PluginPageProps) {
    return (
      <main style={{ display: "grid", gap: 16, maxWidth: 920 }}>
        ...
        <section style={panelStyle}><h2 style={{ margin: "0 0 12px", fontSize: 18 }}>Available Tool Groups</h2><div style={{ display: "grid", gap: 8, fontSize: 14 }}><div>{definition.description}</div><div>Core tools include read/search/list plus dry-run protected write operations.</div><div>Keep dry run enabled while agents prepare developer platform changes.</div><div>These tools operate developer tools and platform resources; review production changes before live execution.</div></div></section>
      </main>
    );
  }

  function DeveloperSettingsPage(_props: PluginSettingsPageProps) {
  ...
  return { DeveloperDashboardWidget, DeveloperPage, DeveloperSettingsPage };
}
