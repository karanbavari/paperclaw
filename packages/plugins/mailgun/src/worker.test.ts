...
describe("Mailgun communication plugin", () => {
  it("declares communication category and core tools", () => {
    expect(manifest.categories).toContain("communication");
    expect(manifest.tools?.map((tool) => tool.name)).toContain("mailgun.apiRequest");
    expect(definition.packageName).toBe("@kesarcloud/plugin-mailgun");
  });
  ...
  it("prepares mutating requests without calling external APIs in dry-run mode", async () => {
    ...
    const result = await harness.executeTool("mailgun.messageSend", {
      "domain": "example.com",
      "message": {
            "to": "+15551234567",
            "text": "Example"
