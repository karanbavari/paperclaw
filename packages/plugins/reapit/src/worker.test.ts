...
describe("Reapit Foundations real estate plugin", () => {
  it("declares real estate category and core tools", () => {
    expect(manifest.categories).toContain("real-estate");
    expect(manifest.tools?.map((tool) => tool.name)).toContain("reapit.apiRequest");
    expect(definition.packageName).toBe("@kesarcloud/plugin-reapit");
  });
  ...
  it("prepares mutating requests without calling external APIs in dry-run mode", async () => {
    ...
    const result = await harness.executeTool("reapit.contactCreate", { "contact": { name: "Example" } }, runCtx);
