...
describe("Bland AI communication plugin", () => {
  it("declares communication category and core tools", () => {
    expect(manifest.categories).toContain("communication");
    expect(manifest.tools?.map((tool) => tool.name)).toContain("bland-ai.apiRequest");
    expect(definition.packageName).toBe("@kesarcloud/plugin-bland-ai");
  });
  ...
  it("prepares mutating requests without calling external APIs in dry-run mode", async () => {
    ...
    const result = await harness.executeTool("bland-ai.callCreate", {
      "call": {
            "to": "+15551234567",
            "text": "Example"
      }
