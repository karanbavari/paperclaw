...
describe("Google RCS Business Messaging communication plugin", () => {
  it("declares communication category and core tools", () => {
    expect(manifest.categories).toContain("communication");
    expect(manifest.tools?.map((tool) => tool.name)).toContain("google-rcs-business-messaging.apiRequest");
    expect(definition.packageName).toBe("@kesarcloud/plugin-google-rcs-business-messaging");
  });
  ...
  it("prepares mutating requests without calling external APIs in dry-run mode", async () => {
    ...
    const result = await harness.executeTool("google-rcs-business-messaging.messageSend", {
