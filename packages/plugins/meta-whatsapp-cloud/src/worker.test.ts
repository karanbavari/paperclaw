...
describe("Meta WhatsApp Cloud API communication plugin", () => {
  it("declares communication category and core tools", () => {
    expect(manifest.categories).toContain("communication");
    expect(manifest.tools?.map((tool) => tool.name)).toContain("meta-whatsapp-cloud.apiRequest");
    expect(definition.packageName).toBe("@kesarcloud/plugin-meta-whatsapp-cloud");
  });
  ...
  it("prepares mutating requests without calling external APIs in dry-run mode", async () => {
    ...
    const result = await harness.executeTool("meta-whatsapp-cloud.messageSend", {
