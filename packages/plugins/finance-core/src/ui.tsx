...
import { financeActionKeys, financeDataKeys, type FinanceDefinition } from "./shared.js";
...
export function createFinanceUi(definition: FinanceDefinition, pluginId: string) {
  ...
  function StatusPanel({ compact = false }: { compact?: boolean }) {
    ...
    const status = usePluginData<StatusData>(financeDataKeys.status, { companyId: host.companyId });
  ...
  function RecentCommands() {
    const host = useHostContext();
    const recent = usePluginData<RecentCommandsData>(financeDataKeys.recentCommands, { companyId: host.companyId });
  ...
  function SetupForm() {
    const host = useHostContext();
    const saveAccessToken = usePluginAction(financeActionKeys.saveAccessToken);
    const saveClientSecret = usePluginAction(financeActionKeys.saveClientSecret);
    const startOauth = usePluginAction(financeActionKeys.startOauth);
    const completeOauth = usePluginAction(financeActionKeys.completeOauth);
    const status = usePluginData<StatusData>(financeDataKeys.status, { companyId: host.companyId });
  ...
  function FinanceDashboardWidget(_props: PluginWidgetProps) {
    return <section style={{ display: "grid", gap: 12 }}><StatusPanel compact /><RecentCommands /></section>;
  }

  function FinancePage(_props: PluginPageProps) {
    return (
      <main style={{ display: "grid", gap: 16, maxWidth: 920 }}>
        ...
        <section style={panelStyle}><h2 style={{ margin: "0 0 12px", fontSize: 18 }}>Available Tool Groups</h2><div style={{ display: "grid", gap: 8, fontSize: 14 }}><div>{definition.description}</div><div>Core tools include read/search/list plus dry-run protected write operations.</div><div>Keep dry run enabled while agents prepare finance workspace changes.</div><div>These tools operate finance records and transactions; they do not provide accounting, tax, investment, or legal advice.</div></div></section>
      </main>
    );
  }

  function FinanceSettingsPage(_props: PluginSettingsPageProps) {
  ...
  return { FinanceDashboardWidget, FinancePage, FinanceSettingsPage };
}
