...
import { financeActionKeys, financeDataKeys, type FinanceDefinition, type FinanceAuthMode, type FinanceEndpoint } from "./shared.js";

export { financeActionKeys, financeDataKeys, type FinanceDefinition, type FinanceEndpoint } from "./shared.js";

export type FinanceConfig = {
  authMode: FinanceAuthMode;
...
export type FinanceRequestPlan = {
  operation: string;
...
export function dataKey(definition: FinanceDefinition, suffix: string) {
...
function normalizeConfig(definition: FinanceDefinition, raw: Record<string, unknown>): FinanceConfig {
  return {
    ...
    allowedCurrencies: Array.isArray(raw.allowedCurrencies) ? raw.allowedCurrencies.map((currency) => String(currency).trim().toUpperCase()).filter(Boolean) : ["USD", "INR", "EUR", "GBP", "AUD", "CAD"],
    maxAmountSubunits: asNumber(raw.maxAmountSubunits, 10_000_000, 0, 1_000_000_000_000),
...
function validateConfig(definition: FinanceDefinition, raw: Record<string, unknown>) {
  ...
  warnings.push(`${definition.displayName} tools automate finance operations data only; they do not provide accounting, tax, investment, or legal advice.`);
  return { config, errors, warnings };
}

function isConnected(config: FinanceConfig) {
...
function toolName(definition: FinanceDefinition, key: string) {
  return `${definition.routePath}.${key}`;
}

function manifestParameters(endpoint: FinanceEndpoint) {
...
export function createFinanceManifest(definition: FinanceDefinition): PaperClawPluginManifestV1 {
  return {
    id: definition.id,
    apiVersion: 1,
    ...
    author: "PaperClaw",
    categories: ["finance", "connector", "automation", "workspace"],
    capabilities: [
      "http.outbound",
      "secrets.read-ref",
      "secrets.write-ref",
    ...
    instanceConfigSchema: {
      ...
      properties: {
        ...
        allowedCurrencies: { type: "array", title: "Allowed Currencies", default: ["USD", "INR", "EUR", "GBP", "AUD", "CAD"], items: { type: "string" } },
        maxAmountSubunits: { type: "number", title: "Max Amount Subunits", default: 10_000_000, minimum: 0 },
    ...
    ui: {
      slots: [
        { type: "page", id: `${definition.routePath}-page`, displayName: definition.displayName, exportName: "FinancePage", routePath: definition.routePath, order: 70 },
        { type: "settingsPage", id: `${definition.routePath}-settings-page`, displayName: definition.displayName, exportName: "FinanceSettingsPage", order: 70 },
        { type: "dashboardWidget", id: `${definition.routePath}-dashboard-widget`, displayName: definition.displayName, exportName: "FinanceDashboardWidget", order: 70 },
      ],
...
function buildPlan(definition: FinanceDefinition, name: string, params: Params, config: FinanceConfig): FinanceRequestPlan {
  ...
  if (name === rawTool) {
    ...
    return {
      ...
      method: method as FinanceRequestPlan["method"],
      path,
...
async function tokenRequest(ctx: PluginContext, definition: FinanceDefinition, config: FinanceConfig, body: Record<string, string>): Promise<TokenResponse> {
...
async function resolveAccessToken(ctx: PluginContext, definition: FinanceDefinition, config: FinanceConfig, companyId: string) {
...
async function request(ctx: PluginContext, definition: FinanceDefinition, config: FinanceConfig, plan: FinanceRequestPlan, companyId: string) {
...
function collectFinanceGuardValues(value: unknown, output: { amounts: number[]; currencies: string[] }) {
  if (Array.isArray(value)) {
    for (const item of value) collectFinanceGuardValues(item, output);
    return output;
  }
  if (!value || typeof value !== "object") return output;
  for (const [key, child] of Object.entries(value as Params)) {
    const normalizedKey = key.toLowerCase();
    if (["amount", "unit_amount", "amount_to_capture", "amount_subunits"].includes(normalizedKey)) {
      const parsed = typeof child === "number" ? child : Number.parseInt(String(child ?? ""), 10);
      if (Number.isFinite(parsed)) output.amounts.push(Math.abs(Math.floor(parsed)));
    }
    if (normalizedKey.includes("currency") && typeof child === "string" && child.trim()) {
      output.currencies.push(child.trim().toUpperCase());
    }
    collectFinanceGuardValues(child, output);
  }
  return output;
}

function guardPlan(config: FinanceConfig, plan: FinanceRequestPlan) {
  if (plan.mutating && !config.allowedOperations.includes("*") && plan.operationKey && !config.allowedOperations.includes(plan.operationKey)) {
    throw new Error(`${plan.operation} is not allowed by finance plugin settings.`);
  }
  const values = collectFinanceGuardValues({ query: plan.query, body: plan.body }, { amounts: [], currencies: [] });
  const largestAmount = Math.max(0, ...values.amounts);
  if (largestAmount > config.maxAmountSubunits) {
    throw new Error(`${plan.operation} amount ${largestAmount} exceeds the configured max amount ${config.maxAmountSubunits}.`);
  }
  const disallowedCurrency = values.currencies.find((currency) => !config.allowedCurrencies.includes(currency));
  if (disallowedCurrency) {
    throw new Error(`${plan.operation} currency ${disallowedCurrency} is not allowed by finance plugin settings.`);
  }
}
...
async function audit(ctx: PluginContext, definition: FinanceDefinition, runCtx: ToolRunContext, plan: FinanceRequestPlan, result: { ok: boolean; dryRun: boolean }) {
...
async function executePlan(ctx: PluginContext, definition: FinanceDefinition, runCtx: ToolRunContext, plan: FinanceRequestPlan): Promise<ToolResult> {
...
export function createFinancePlugin(definition: FinanceDefinition) {
  return definePlugin({
    async setup(ctx) {
      ...
      ctx.data.register(financeDataKeys.status, async (params) => {
      ...
      ctx.data.register(financeDataKeys.recentCommands, async (params) => {
        const companyId = stringParam(asObject(params), "companyId");
      ...
      ctx.actions.register(financeActionKeys.saveAccessToken, async (params) => ctx.secrets.upsert({
      ...
      ctx.actions.register(financeActionKeys.saveClientSecret, async (params) => ctx.secrets.upsert({
        companyId: requireString(asObject(params), "companyId"),
      ...
      ctx.actions.register(financeActionKeys.startOauth, async (params) => {
      ...
      ctx.actions.register(financeActionKeys.completeOauth, async (params) => {
      ...
      ctx.actions.register(financeActionKeys.disconnect, async (params) => {
        const companyId = stringParam(asObject(params), "companyId");
...
export function runFinanceWorker(definition: FinanceDefinition, importMetaUrl: string) {
  runWorker(createFinancePlugin(definition), importMetaUrl);
}
