...
function StatusPanel({ compact = false }: { compact?: boolean }) {
  ...
  if (status.loading) return <div style={mutedStyle}>Checking ecommerce connector...</div>;
    ...
    <div style={{ display: "grid", gap: 8 }}>
      ...
      {!compact ? (
        <>
          ...
          <div style={mutedStyle}>Store/project: {data?.storeId || "Not required"}</div>
...
function RecentCommands() {
  ...
  if (recent.loading) return <div style={mutedStyle}>Loading recent ecommerce commands...</div>;
  if (recent.error) return <div style={mutedStyle}>Recent command audit is unavailable.</div>;
  const commands = recent.data?.commands ?? [];
  if (commands.length === 0) return <div style={mutedStyle}>No ecommerce commands have been run yet.</div>;
  return (
    <div style={{ display: "grid", gap: 8 }}>
...
export function EcommerceDashboardWidget(_props: PluginWidgetProps) {
  return (
...
export function EcommercePage(_props: PluginPageProps) {
  const host = useHostContext();
  const status = usePluginData<StatusData>("status", { companyId: host.companyId });
  const title = status.data?.displayName ?? "Ecommerce";
  return (
    <main style={{ display: "grid", gap: 16, padding: 24 }}>
      <header>
        <h1 style={{ margin: 0, fontSize: 22 }}>{title}</h1>
        <p style={{ ...mutedStyle, marginTop: 6 }}>Store, catalog, inventory, order, customer, and webhook tools for PaperClaw agents.</p>
      </header>
...
export function EcommerceSettingsPage(_props: PluginSettingsPageProps) {
  const host = useHostContext();
  const status = usePluginData<StatusData>("status", { companyId: host.companyId });
  return (
    <section style={{ display: "grid", gap: 16 }}>
      ...
      <div style={panelStyle}>
        ...
        <div style={mutedStyle}>{status.data?.setupNotes ?? "Configure credentials and keep dry run enabled until the first test succeeds."}</div>
      </div>
      <div style={panelStyle}>
        <h2 style={{ margin: "0 0 8px", fontSize: 16 }}>Agent guardrails</h2>
        <div style={mutedStyle}>Mutating tools respect dry-run mode, operation allowlists, output limits, and activity logging.</div>
      </div>
    </section>
  );
}
