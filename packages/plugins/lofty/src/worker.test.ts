...
describe("Lofty real estate plugin", () => {
  it("declares real estate category and core tools", () => {
    expect(manifest.categories).toContain("real-estate");
    expect(manifest.tools?.map((tool) => tool.name)).toContain("lofty.apiRequest");
    expect(definition.packageName).toBe("@kesarcloud/plugin-lofty");
  });
  ...
  it("prepares mutating requests without calling external APIs in dry-run mode", async () => {
    ...
    const result = await harness.executeTool("lofty.leadCreate", { "lead": { name: "Example" } }, runCtx);
