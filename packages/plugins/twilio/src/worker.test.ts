...
describe("Twilio communication plugin", () => {
  it("declares communication category and core tools", () => {
    expect(manifest.categories).toContain("communication");
    expect(manifest.tools?.map((tool) => tool.name)).toContain("twilio.apiRequest");
    expect(definition.packageName).toBe("@kesarcloud/plugin-twilio");
  });
  ...
  it("prepares mutating requests without calling external APIs in dry-run mode", async () => {
    ...
    const result = await harness.executeTool("twilio.messageCreate", {
