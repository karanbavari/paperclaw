...
import { communicationActionKeys, communicationDataKeys, type CommunicationDefinition, type CommunicationAuthMode, type CommunicationEndpoint } from "./shared.js";

export { communicationActionKeys, communicationDataKeys, type CommunicationDefinition, type CommunicationEndpoint } from "./shared.js";

export type CommunicationConfig = {
  authMode: CommunicationAuthMode;
...
export type CommunicationRequestPlan = {
  operation: string;
...
export function dataKey(definition: CommunicationDefinition, suffix: string) {
...
function normalizeConfig(definition: CommunicationDefinition, raw: Record<string, unknown>): CommunicationConfig {
  return {
...
function validateConfig(definition: CommunicationDefinition, raw: Record<string, unknown>) {
  ...
  warnings.push(`${definition.displayName} tools automate communication operating data only; agents must verify compliance, licensing, and channel policy, consent, opt-out, template, and local telecom rules before live use.`);
  return { config, errors, warnings };
}

function isConnected(config: CommunicationConfig) {
...
function toolName(definition: CommunicationDefinition, key: string) {
  return `${definition.routePath}.${key}`;
}

function manifestParameters(endpoint: CommunicationEndpoint) {
...
export function createCommunicationManifest(definition: CommunicationDefinition): PaperClawPluginManifestV1 {
  return {
    id: definition.id,
    apiVersion: 1,
    ...
    author: "PaperClaw",
    categories: ["communication", "connector", "automation", "workspace"],
    capabilities: [
      "http.outbound",
      "secrets.read-ref",
      "secrets.write-ref",
    ...
    ui: {
      slots: [
        { type: "page", id: `${definition.routePath}-page`, displayName: definition.displayName, exportName: "CommunicationPage", routePath: definition.routePath, order: 70 },
        { type: "settingsPage", id: `${definition.routePath}-settings-page`, displayName: definition.displayName, exportName: "CommunicationSettingsPage", order: 70 },
        { type: "dashboardWidget", id: `${definition.routePath}-dashboard-widget`, displayName: definition.displayName, exportName: "CommunicationDashboardWidget", order: 70 },
      ],
...
function buildPlan(definition: CommunicationDefinition, name: string, params: Params, config: CommunicationConfig): CommunicationRequestPlan {
  ...
  if (name === rawTool) {
    ...
    return {
      ...
      method: method as CommunicationRequestPlan["method"],
      path,
...
async function tokenRequest(ctx: PluginContext, definition: CommunicationDefinition, config: CommunicationConfig, body: Record<string, string>): Promise<TokenResponse> {
...
async function resolveAccessToken(ctx: PluginContext, definition: CommunicationDefinition, config: CommunicationConfig, companyId: string) {
...
async function request(ctx: PluginContext, definition: CommunicationDefinition, config: CommunicationConfig, plan: CommunicationRequestPlan, companyId: string) {
  ...
  } else if (authScheme === "basic-token") {
    headers.authorization = `Basic ${Buffer.from(`${accessToken}:`, "utf8").toString("base64")}`;
  } else if (authScheme === "basic-pair") {
    headers.authorization = `Basic ${Buffer.from(accessToken, "utf8").toString("base64")}`;
  } else {
...
async function audit(ctx: PluginContext, definition: CommunicationDefinition, runCtx: ToolRunContext, plan: CommunicationRequestPlan, result: { ok: boolean; dryRun: boolean }) {
...
async function executePlan(ctx: PluginContext, definition: CommunicationDefinition, runCtx: ToolRunContext, plan: CommunicationRequestPlan): Promise<ToolResult> {
...
export function createCommunicationPlugin(definition: CommunicationDefinition) {
  return definePlugin({
    async setup(ctx) {
      ...
      for (const name of toolNames) {
        ctx.tools.register(name, { displayName: name, description: `Run ${name}`, parametersSchema: { type: "object" } }, async (rawParams, runCtx) => {
          let plan: CommunicationRequestPlan;
          try {
            const config = normalizeConfig(definition, await ctx.config.get());
      ...
      ctx.data.register(communicationDataKeys.status, async (params) => {
      ...
      ctx.data.register(communicationDataKeys.recentCommands, async (params) => {
        const companyId = stringParam(asObject(params), "companyId");
      ...
      ctx.actions.register(communicationActionKeys.saveAccessToken, async (params) => ctx.secrets.upsert({
      ...
      ctx.actions.register(communicationActionKeys.saveClientSecret, async (params) => ctx.secrets.upsert({
        companyId: requireString(asObject(params), "companyId"),
      ...
      ctx.actions.register(communicationActionKeys.startOauth, async (params) => {
      ...
      ctx.actions.register(communicationActionKeys.completeOauth, async (params) => {
      ...
      ctx.actions.register(communicationActionKeys.disconnect, async (params) => {
        const companyId = stringParam(asObject(params), "companyId");
...
export function runCommunicationWorker(definition: CommunicationDefinition, importMetaUrl: string) {
  runWorker(createCommunicationPlugin(definition), importMetaUrl);
}
