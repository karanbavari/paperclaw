...
import { communicationActionKeys, communicationDataKeys, type CommunicationDefinition } from "./shared.js";
...
export function createCommunicationUi(definition: CommunicationDefinition, pluginId: string) {
  ...
  function StatusPanel({ compact = false }: { compact?: boolean }) {
    ...
    const status = usePluginData<StatusData>(communicationDataKeys.status, { companyId: host.companyId });
  ...
  function RecentCommands() {
    const host = useHostContext();
    const recent = usePluginData<RecentCommandsData>(communicationDataKeys.recentCommands, { companyId: host.companyId });
  ...
  function SetupForm() {
    const host = useHostContext();
    const saveAccessToken = usePluginAction(communicationActionKeys.saveAccessToken);
    const saveClientSecret = usePluginAction(communicationActionKeys.saveClientSecret);
    const startOauth = usePluginAction(communicationActionKeys.startOauth);
    const completeOauth = usePluginAction(communicationActionKeys.completeOauth);
    const status = usePluginData<StatusData>(communicationDataKeys.status, { companyId: host.companyId });
  ...
  function CommunicationDashboardWidget(_props: PluginWidgetProps) {
    return <section style={{ display: "grid", gap: 12 }}><StatusPanel compact /><RecentCommands /></section>;
  }

  function CommunicationPage(_props: PluginPageProps) {
    return (
      <main style={{ display: "grid", gap: 16, maxWidth: 920 }}>
        ...
        <section style={panelStyle}><h2 style={{ margin: "0 0 12px", fontSize: 18 }}>Available Tool Groups</h2><div style={{ display: "grid", gap: 8, fontSize: 14 }}><div>{definition.description}</div><div>Core tools include read/search/list plus dry-run protected send, call, and template operations.</div><div>Keep dry run enabled while agents prepare communication workspace changes.</div><div>Verify consent, opt-out, template, recording, and local telecom rules before live use.</div></div></section>
      </main>
    );
  }

  function CommunicationSettingsPage(_props: PluginSettingsPageProps) {
  ...
  return { CommunicationDashboardWidget, CommunicationPage, CommunicationSettingsPage };
}
