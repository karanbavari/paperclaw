...
import { productivityActionKeys, productivityDataKeys, type ProductivityDefinition } from "./shared.js";
...
export function createProductivityUi(definition: ProductivityDefinition, pluginId: string) {
  ...
  function StatusPanel({ compact = false }: { compact?: boolean }) {
    ...
    const status = usePluginData<StatusData>(productivityDataKeys.status, { companyId: host.companyId });
  ...
  function RecentCommands() {
    const host = useHostContext();
    const recent = usePluginData<RecentCommandsData>(productivityDataKeys.recentCommands, { companyId: host.companyId });
  ...
  function SetupForm() {
    const host = useHostContext();
    const saveAccessToken = usePluginAction(productivityActionKeys.saveAccessToken);
    const saveClientSecret = usePluginAction(productivityActionKeys.saveClientSecret);
    const startOauth = usePluginAction(productivityActionKeys.startOauth);
    const completeOauth = usePluginAction(productivityActionKeys.completeOauth);
    const status = usePluginData<StatusData>(productivityDataKeys.status, { companyId: host.companyId });
  ...
  function ProductivityDashboardWidget(_props: PluginWidgetProps) {
    return <section style={{ display: "grid", gap: 12 }}><StatusPanel compact /><RecentCommands /></section>;
  }

  function ProductivityPage(_props: PluginPageProps) {
    return (
      <main style={{ display: "grid", gap: 16, maxWidth: 920 }}>
        ...
        <section style={panelStyle}><h2 style={{ margin: "0 0 12px", fontSize: 18 }}>Available Tool Groups</h2><div style={{ display: "grid", gap: 8, fontSize: 14 }}><div>{definition.description}</div><div>Core tools include read/search/list plus dry-run protected write operations.</div><div>Keep dry run enabled while agents prepare productivity workspace changes.</div></div></section>
      </main>
    );
  }

  function ProductivitySettingsPage(_props: PluginSettingsPageProps) {
  ...
  return { ProductivityDashboardWidget, ProductivityPage, ProductivitySettingsPage };
}
