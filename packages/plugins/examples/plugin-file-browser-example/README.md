...
Example PaperClaw plugin that demonstrates:
...
```bash
pnpm --filter @kesarcloud/plugin-file-browser-example build
pnpm paperclaw plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm paperclaw plugin uninstall paperclaw-file-browser-example --force
```
...
- Optional: use `paperclaw-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
