...
type PaperClawSprite = (typeof PAPERCLAW_SPRITES)[number];
...
function spriteSize(sprite: PaperClawSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = PAPERCLAW_SPRITES[(baseCol + baseRow) % PAPERCLAW_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = PAPERCLAW_SPRITES[Math.floor(Math.random() * PAPERCLAW_SPRITES.length)]!;
