# @kesarcloud/plugin-hello-world-example
...
- a minimal React widget rendered in the PaperClaw dashboard
...
```bash
pnpm --filter @kesarcloud/plugin-hello-world-example build
pnpm paperclaw plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `pnpm paperclaw plugin uninstall paperclaw.hello-world-example --force` then  
  `pnpm paperclaw plugin install ./packages/plugins/examples/plugin-hello-world-example`.
