...
describe("Exotel communication plugin", () => {
  it("declares communication category and core tools", () => {
    expect(manifest.categories).toContain("communication");
    expect(manifest.tools?.map((tool) => tool.name)).toContain("exotel.apiRequest");
    expect(definition.packageName).toBe("@kesarcloud/plugin-exotel");
  });
  ...
  it("prepares mutating requests without calling external APIs in dry-run mode", async () => {
    ...
    const result = await harness.executeTool("exotel.callCreate", {
