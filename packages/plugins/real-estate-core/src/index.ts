...
import { realEstateActionKeys, realEstateDataKeys, type RealEstateDefinition, type RealEstateAuthMode, type RealEstateEndpoint } from "./shared.js";

export { realEstateActionKeys, realEstateDataKeys, type RealEstateDefinition, type RealEstateEndpoint } from "./shared.js";

export type RealEstateConfig = {
  authMode: RealEstateAuthMode;
...
export type RealEstateRequestPlan = {
  operation: string;
...
export function dataKey(definition: RealEstateDefinition, suffix: string) {
...
function normalizeConfig(definition: RealEstateDefinition, raw: Record<string, unknown>): RealEstateConfig {
  return {
...
function validateConfig(definition: RealEstateDefinition, raw: Record<string, unknown>) {
  ...
  warnings.push(`${definition.displayName} tools automate real estate operating data only; agents must verify compliance, licensing, and local market rules before live use.`);
  return { config, errors, warnings };
}

function isConnected(config: RealEstateConfig) {
...
function toolName(definition: RealEstateDefinition, key: string) {
  return `${definition.routePath}.${key}`;
}

function manifestParameters(endpoint: RealEstateEndpoint) {
...
export function createRealEstateManifest(definition: RealEstateDefinition): PaperClawPluginManifestV1 {
  return {
    id: definition.id,
    apiVersion: 1,
    ...
    author: "PaperClaw",
    categories: ["real-estate", "connector", "automation", "workspace"],
    capabilities: [
      "http.outbound",
      "secrets.read-ref",
      "secrets.write-ref",
    ...
    ui: {
      slots: [
        { type: "page", id: `${definition.routePath}-page`, displayName: definition.displayName, exportName: "RealEstatePage", routePath: definition.routePath, order: 70 },
        { type: "settingsPage", id: `${definition.routePath}-settings-page`, displayName: definition.displayName, exportName: "RealEstateSettingsPage", order: 70 },
        { type: "dashboardWidget", id: `${definition.routePath}-dashboard-widget`, displayName: definition.displayName, exportName: "RealEstateDashboardWidget", order: 70 },
      ],
...
function buildPlan(definition: RealEstateDefinition, name: string, params: Params, config: RealEstateConfig): RealEstateRequestPlan {
  ...
  if (name === rawTool) {
    ...
    return {
      ...
      method: method as RealEstateRequestPlan["method"],
      path,
...
async function tokenRequest(ctx: PluginContext, definition: RealEstateDefinition, config: RealEstateConfig, body: Record<string, string>): Promise<TokenResponse> {
...
async function resolveAccessToken(ctx: PluginContext, definition: RealEstateDefinition, config: RealEstateConfig, companyId: string) {
...
async function request(ctx: PluginContext, definition: RealEstateDefinition, config: RealEstateConfig, plan: RealEstateRequestPlan, companyId: string) {
...
async function audit(ctx: PluginContext, definition: RealEstateDefinition, runCtx: ToolRunContext, plan: RealEstateRequestPlan, result: { ok: boolean; dryRun: boolean }) {
...
async function executePlan(ctx: PluginContext, definition: RealEstateDefinition, runCtx: ToolRunContext, plan: RealEstateRequestPlan): Promise<ToolResult> {
...
export function createRealEstatePlugin(definition: RealEstateDefinition) {
  return definePlugin({
    async setup(ctx) {
      ...
      for (const name of toolNames) {
        ctx.tools.register(name, { displayName: name, description: `Run ${name}`, parametersSchema: { type: "object" } }, async (rawParams, runCtx) => {
          let plan: RealEstateRequestPlan;
          try {
            const config = normalizeConfig(definition, await ctx.config.get());
      ...
      ctx.data.register(realEstateDataKeys.status, async (params) => {
      ...
      ctx.data.register(realEstateDataKeys.recentCommands, async (params) => {
        const companyId = stringParam(asObject(params), "companyId");
      ...
      ctx.actions.register(realEstateActionKeys.saveAccessToken, async (params) => ctx.secrets.upsert({
      ...
      ctx.actions.register(realEstateActionKeys.saveClientSecret, async (params) => ctx.secrets.upsert({
        companyId: requireString(asObject(params), "companyId"),
      ...
      ctx.actions.register(realEstateActionKeys.startOauth, async (params) => {
      ...
      ctx.actions.register(realEstateActionKeys.completeOauth, async (params) => {
      ...
      ctx.actions.register(realEstateActionKeys.disconnect, async (params) => {
        const companyId = stringParam(asObject(params), "companyId");
...
export function runRealEstateWorker(definition: RealEstateDefinition, importMetaUrl: string) {
  runWorker(createRealEstatePlugin(definition), importMetaUrl);
}
