...
export function resolveManagedCodexHomeDir(
  ...
  return companyId
    ? path.resolve(paperclawHome, "instances", instanceId, "companies", companyId, "codex-home")
    : path.resolve(paperclawHome, "instances", instanceId, "codex-home");
}
...
export async function prepareManagedCodexHome(
  ...
  if (seedFromShared) {
    ...
    await onLog(
      "stdout",
      `[paperclaw] Using ${isWorktreeMode(env) ? "worktree-isolated" : "PaperClaw-managed"} Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
    );
  }
  ...
  if (apiKey) {
    ...
    await onLog(
      "stdout",
      `[paperclaw] Wrote API-key auth.json into Codex home "${targetHome}" from configured OPENAI_API_KEY.\n`,
    );
  }
