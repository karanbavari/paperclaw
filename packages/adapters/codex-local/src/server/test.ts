    ...
    if (!commandLooksLike(command, "codex")) {
      ...
      if (probeApiKey) {
        const probeHome = targetIsRemote
          ? `/tmp/paperclaw-codex-probe-${runId}`
          : path.join(os.tmpdir(), `paperclaw-codex-probe-${runId}`);
        probeEnv.CODEX_HOME = probeHome;
        probeEnv._PAPERCLAW_CODEX_AUTH_JSON = JSON.stringify({ OPENAI_API_KEY: probeApiKey });
        probeCommand = "sh";
        ...
        probeArgs = [
          "-c",
          'set -e; mkdir -p "$CODEX_HOME"; umask 077; printf "%s" "$_PAPERCLAW_CODEX_AUTH_JSON" > "$CODEX_HOME/auth.json"; unset _PAPERCLAW_CODEX_AUTH_JSON; trap \'rm -rf "$CODEX_HOME"\' EXIT INT TERM; "$0" "$@"',
          command,
          ...args,
        ];
      }
      ...
      if (probe.timedOut) {
        ...
        checks.push({
          ...
          hint: probeApiKey
            ...
            : "Codex CLI does not read OPENAI_API_KEY from the environment; set OPENAI_API_KEY in this adapter's config (so PaperClaw writes it to `$CODEX_HOME/auth.json`) or run `codex login` on the host first.",
        });
      } else {
        checks.push({
