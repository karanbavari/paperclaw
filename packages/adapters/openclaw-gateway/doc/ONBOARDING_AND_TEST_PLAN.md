...
## Scope
This plan is now **gateway-only**. PaperClaw supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses PaperClaw skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a PaperClaw task.
...
```bash
pnpm --filter @kesarcloud/server test:run -- openclaw-gateway
pnpm --filter @kesarcloud/server typecheck
pnpm --filter @kesarcloud/ui typecheck
pnpm --filter paperclaw typecheck
```
