...
Use when:
- You want PaperClaw to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the PaperClaw server.
...
- paperclawApiUrl (string, optional): absolute PaperClaw base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/paperclaw-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default paperclaw)

Standard outbound payload additions:
- paperclaw (object): standardized PaperClaw context added to every gateway agent request
- paperclaw.workspace (object, optional): resolved execution workspace for this run
- paperclaw.workspaces (array, optional): additional workspace hints PaperClaw exposed to the run
- paperclaw.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
