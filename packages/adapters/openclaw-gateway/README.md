...
This document describes how `@kesarcloud/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into PaperClaw logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (PaperClaw `runId`)
