...
import { asString, runChildProcess } from "@kesarcloud/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.PAPERCLAW_PI_COMMAND === "string" &&
    process.env.PAPERCLAW_PI_COMMAND.trim().length > 0
      ? process.env.PAPERCLAW_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["PAPERCLAW_", "npm_", "NPM_"] as const;
