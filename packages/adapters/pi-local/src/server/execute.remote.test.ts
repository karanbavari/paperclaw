...
describe("pi remote execution", () => {
  ...
  it("prepares the workspace, syncs Pi skills, and restores workspace changes for remote SSH execution", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclaw-pi-remote-"));
    ...
    expect(String(result.sessionId)).toContain("/remote/workspace/.paperclaw-runtime/pi/sessions/");
    expect(prepareWorkspaceForSshExecution).toHaveBeenCalledTimes(1);
    expect(syncDirectoryToSsh).toHaveBeenCalledTimes(1);
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: "/remote/workspace/.paperclaw-runtime/pi/skills",
      followSymlinks: true,
    }));
    ...
    expect(call?.[2]).toContain("/remote/workspace/.paperclaw-runtime/pi/skills");
  ...
  it("resumes saved Pi sessions for remote SSH execution only when the identity matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclaw-pi-remote-resume-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    expect(call?.[2]).toContain("--session");
    expect(call?.[2]).toContain("/remote/workspace/.paperclaw-runtime/pi/sessions/session-123.jsonl");
  });

  it("starts a fresh remote Pi session when the saved session header cwd points at a different workspace", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclaw-pi-remote-stale-session-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    expect(usedSession).toContain("/remote/workspace/.paperclaw-runtime/pi/sessions/");
  ...
  it("starts a fresh remote Pi session when the saved session header is empty or unreadable", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclaw-pi-remote-empty-header-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    await execute({
      ...
      context: {
        paperclawWorkspace: { cwd: workspaceDir, source: "project_primary" },
      },
      executionTransport: {
        remoteExecution: {
          host: "127.0.0.1",
  ...
  it("starts a fresh remote Pi session when the remote head command fails", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "paperclaw-pi-remote-head-failure-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
