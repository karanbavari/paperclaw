...
async function buildClaudeSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  const entries: AdapterSkillEntry[] = availableEntries.map((entry) => ({
    ...
    detail: desiredSet.has(entry.key)
      ? "Will be materialized into the stable PaperClaw-managed Claude prompt bundle on the next run."
      : null,
...
export function resolveClaudeDesiredSkillNames(
  ...
  return resolvePaperClawDesiredSkillNames(config, availableEntries);
}
