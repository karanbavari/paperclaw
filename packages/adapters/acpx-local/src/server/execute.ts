...
function defaultPaperClawInstanceDir(): string {
...
function defaultStateDir(companyId: string, agentId: string): string {
  return path.join(defaultPaperClawInstanceDir(), "companies", companyId, "acpx-local", "agents", agentId);
}

function resolveManagedCodexHomeDir(companyId: string): string {
  return path.join(defaultPaperClawInstanceDir(), "companies", companyId, "codex-home");
}
...
async function prepareManagedCodexHome(input: {
  ...
  await onLog(
    "stdout",
    `[paperclaw] Using PaperClaw-managed ACPX Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
...
async function buildSkillSetKey(input: {
  ...
  hash.update(`paperclaw-acpx-${input.label}-skills:v1\n`);
...
async function prepareClaudeSkillRuntime(input: {
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[paperclaw] Materialized ACPX Claude skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[paperclaw] Failed to materialize ACPX Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
  ...
  const promptInstructions = selectedSkills.length > 0
    ? [
        "PaperClaw has materialized selected runtime skills for this ACPX Claude session.",
  ...
  return {
    ...
    commandNotes: selectedSkills.length > 0
      ? [`Materialized ${selectedSkills.length} PaperClaw skill(s) for ACPX Claude at ${skillsHome}.`]
      : [],
  };
}
...
async function reconcileManagedCodexSkills(input: {
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[paperclaw] Revoked ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const entry of input.allSkills) {
    ...
    if (await removeSkillTarget(target)) {
      await input.onLog("stdout", `[paperclaw] Revoked legacy ACPX Codex skill "${entry.runtimeName}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[paperclaw] Revoked unavailable ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
}
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[paperclaw] Materialized ACPX Codex skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[paperclaw] Failed to inject ACPX Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
async function buildRuntime(input: {
  ...
  if (!hasExplicitApiKey && authToken) env.PAPERCLAW_API_KEY = authToken;
  ...
  const sessionKey = `paperclaw:${agent.companyId}:${agent.id}:${taskKey}:${fingerprint}`;
...
async function applySessionConfigOptions(input: {
  ...
  if (!input.runtime.setConfigOption) {
    ...
    await input.onLog("stderr", `[paperclaw] ${message}\n`);
  ...
  for (const option of options) {
    ...
    await input.onLog(
      "stdout",
      `[paperclaw] Applied ACPX ${input.prepared.acpxAgent} config ${option.key}=${option.value}\n`,
    );
  }
}
...
async function buildPrompt(ctx: AdapterExecutionContext, resumedSession: boolean): Promise<{
  ...
  const promptTemplate = asString(config.promptTemplate, DEFAULT_PAPERCLAW_AGENT_PROMPT_TEMPLATE);
...
export function createAcpxLocalExecutor(deps: ExecuteDeps = {}) {
  ...
  return async function executeAcpxLocal(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    if (!canResume && asString(previousParams.runtimeSessionName, "")) {
      await ctx.onLog(
        "stdout",
        `[paperclaw] ACPX session "${asString(previousParams.runtimeSessionName, "")}" does not match the current agent/cwd/mode/runtime identity; starting fresh in "${prepared.cwd}".\n`,
      );
    }
    ...
    try {
      if (!handle) {
        try {
          ...
          await ctx.onLog(
            "stdout",
            `[paperclaw] ACPX resume session "${resumeSessionId}" is unavailable; retrying with a fresh session.\n`,
          );
          handle = await runtime.ensureSession({
            sessionKey: prepared.sessionKey,
            agent: prepared.acpxAgent,
    ...
    try {
      ...
      await runtime.close({
        handle: sessionHandle,
        reason: "paperclaw config cleanup",
    ...
    if (ctx.onMeta) {
      await ctx.onMeta({
        ...
        cwd: prepared.cwd,
        commandNotes: [
          `ACPX runtime embedded in PaperClaw with ${prepared.mode} session mode.`,
    ...
    try {
      ...
      if (terminal.status === "failed" || terminal.status === "cancelled" || timedOut) {
        ...
        if (warmHandleMatches(existing, runtime, sessionHandle) && existing) {
          await closeWarmHandle({
            ...
            entry: existing,
            reason: timedOut ? "paperclaw timeout cleanup" : `paperclaw turn ${terminal.status}`,
        ...
        if (existing && !warmHandleMatches(existing, runtime, sessionHandle)) {
          await runtime.close({
            handle: sessionHandle,
            reason: "paperclaw duplicate warm handle cleanup",
            discardPersistentState: false,
          }).catch(() => {});
        } else {
        ...
        if (warmHandleMatches(existing, runtime, sessionHandle) && existing) {
          await closeWarmHandle({
            ...
            key: prepared.sessionKey,
            entry: existing,
            reason: "paperclaw completed turn cleanup",
          });
        } else {
          await runtime.close({
            handle: sessionHandle,
      ...
      await runtime.close({
        handle: sessionHandle,
        reason: timedOut ? "paperclaw timeout cleanup" : "paperclaw error cleanup",
