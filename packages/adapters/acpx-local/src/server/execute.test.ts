...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclaw-acpx-skills-"));
  tempRoots.push(root);
  return root;
}
...
describe("acpx_local runtime skill isolation", () => {
  ...
  it("keeps fresh credential wrapper scripts across ACPX agent changes", async () => {
    ...
    await runExecutor({
      ...baseConfig,
      agent: "custom-a",
      env: { PAPERCLAW_API_KEY: "old-key" },
    });
    await runExecutor({
      ...baseConfig,
      agent: "custom-b",
      env: { PAPERCLAW_API_KEY: "new-key" },
    });
    ...
    expect(wrapper).not.toContain("PAPERCLAW_API_KEY");
    expect(wrapper).not.toContain("new-key");
    expect(wrapper).not.toContain("old-key");
    expect(env).toContain("PAPERCLAW_API_KEY='new-key'");
  ...
  it("shapes ACPX wrapper workspace env for remote execution identities", async () => {
    ...
    expect(env).toContain("PAPERCLAW_WORKSPACE_CWD='/remote/workspace'");
    expect(env).not.toContain("PAPERCLAW_WORKSPACE_WORKTREE_PATH=");
  });
  ...
  it("keeps distinct wrapper env files for concurrent runs with different credentials", async () => {
    ...
    await runExecutor({
      ...baseConfig,
      env: { PAPERCLAW_API_KEY: "first-key" },
    });
    await runExecutor({
      ...baseConfig,
      env: { PAPERCLAW_API_KEY: "second-key" },
    });
    ...
    expect(envFiles.filter((contents) => contents.includes("PAPERCLAW_API_KEY='first-key'"))).toHaveLength(1);
    expect(envFiles.filter((contents) => contents.includes("PAPERCLAW_API_KEY='second-key'"))).toHaveLength(1);
  });

  it("passes PaperClaw env through the ACP agent wrapper instead of process.env", async () => {
      ...
      createRuntime: () => ({
        ...
        startTurn: () => ({
          events: (async function* () {
            ...
            observedApiKeyDuringStream = process.env.PAPERCLAW_API_KEY;
            yield { type: "done", stopReason: "end_turn" };
          })(),
          result: Promise.resolve({ status: "completed", stopReason: "end_turn" }),
          cancel: async () => {},
