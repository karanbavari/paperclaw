---
name: paperclaw-create-plugin
description: >
  Create new PaperClaw plugins with the current alpha SDK/runtime. Use when
...
# Create a PaperClaw Plugin

Use this skill when the task is to create, scaffold, or document a PaperClaw plugin.
...
node packages/plugins/create-paperclaw-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the PaperClaw repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.paperclaw-sdk/`:

```bash
pnpm --filter @kesarcloud/create-paperclaw-plugin build
node packages/plugins/create-paperclaw-plugin/dist/index.js @acme/plugin-name \
...
- is installed into PaperClaw from an absolute local path during development
