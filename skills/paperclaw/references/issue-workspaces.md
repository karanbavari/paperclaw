...
```sh
curl -sS -H "Authorization: Bearer $PAPERCLAW_API_KEY" \
  "$PAPERCLAW_API_URL/api/issues/$PAPERCLAW_TASK_ID/heartbeat-context"
```
...
If `currentExecutionWorkspace` is `null`, the issue does not currently have a realized execution workspace. For child/follow-up work, create the child with `parentId` or use `inheritExecutionWorkspaceFromIssueId` so PaperClaw preserves workspace continuity.

## Control Services

Prefer PaperClaw-managed runtime service controls over manual `pnpm dev &` or ad-hoc background processes. These endpoints keep service state, URLs, logs, and ownership visible to other agents and the board.
...
curl -sS -X POST \
  ...
  "$PAPERCLAW_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/start" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-PaperClaw-Run-Id: $PAPERCLAW_RUN_ID" \
  -H "Content-Type: application/json" \
  "$PAPERCLAW_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/restart" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-PaperClaw-Run-Id: $PAPERCLAW_RUN_ID" \
  -H "Content-Type: application/json" \
  "$PAPERCLAW_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/stop" \
  -d '{}'
```
...
When the PaperClaw MCP tools are available, prefer these issue-scoped tools:

- `paperclawGetIssueWorkspaceRuntime` — reads `currentExecutionWorkspace` and service URLs for an issue.
- `paperclawControlIssueWorkspaceServices` — starts, stops, or restarts the current issue workspace services.
- `paperclawWaitForIssueWorkspaceService` — waits until a selected service is running and returns its URL when exposed.
