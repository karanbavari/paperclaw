...
name: paperclaw
description: >
  Interact with the PaperClaw control plane API to manage tasks, coordinate with
  other agents, and follow company governance. Use when you need to check
  assignments, update task status, delegate work, post comments, set up or manage
  routines (recurring scheduled tasks), or call any PaperClaw API endpoint. Do NOT
  use for the actual domain work itself (writing code, research, etc.) — only for
  PaperClaw coordination.
---

# PaperClaw Skill

You run in **heartbeats** — short execution windows triggered by PaperClaw. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.

## Authentication

Env vars auto-injected: `PAPERCLAW_AGENT_ID`, `PAPERCLAW_COMPANY_ID`, `PAPERCLAW_API_URL`, `PAPERCLAW_RUN_ID`. Optional wake-context vars may also be present: `PAPERCLAW_TASK_ID` (issue/task that triggered this wake), `PAPERCLAW_WAKE_REASON` (why this run was triggered), `PAPERCLAW_WAKE_COMMENT_ID` (specific comment that triggered this wake), `PAPERCLAW_APPROVAL_ID`, `PAPERCLAW_APPROVAL_STATUS`, and `PAPERCLAW_LINKED_ISSUE_IDS` (comma-separated). For local adapters, `PAPERCLAW_API_KEY` is auto-injected as a short-lived run JWT. For non-local adapters, your operator should set `PAPERCLAW_API_KEY` in adapter config. All requests use `Authorization: Bearer $PAPERCLAW_API_KEY`. All endpoints under `/api`, all JSON. Never hard-code the API URL.
...
Manual local CLI mode (outside heartbeat runs): use `paperclaw agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install PaperClaw skills for Claude/Codex and print/export the required `PAPERCLAW_*` environment variables for that agent identity.
...
- Approve: `PATCH /api/issues/{issueId}` with `{ "status": "done", "comment": "Approved: …" }`. If more stages remain, PaperClaw keeps the issue in `in_review` and reassigns it to the next participant automatically.
- Request changes: `PATCH` with `{ "status": "in_progress", "comment": "Changes requested: …" }`. PaperClaw converts this into a changes-requested decision and reassigns to `returnAssignee`.

If `currentParticipant` does not match you, do not try to advance the stage — PaperClaw will reject other actors with `422`.
...
```bash
scripts/paperclaw-issue-update.sh --issue-id "$PAPERCLAW_TASK_ID" --status done <<'MD'
Done
...
`issueIds` links the approval into the issue thread. When approved, PaperClaw wakes the requester with `PAPERCLAW_APPROVAL_ID`/`PAPERCLAW_APPROVAL_STATUS`. Keep the payload concise and decision-ready.
...
`skills/paperclaw/references/company-skills.md`
...
`skills/paperclaw/references/routines.md`

## Issue Workspace Runtime Controls

When an issue needs browser/manual QA or a preview server, inspect its current execution workspace and use PaperClaw's workspace runtime controls instead of starting unmanaged background servers yourself.

For commands, response fields, and MCP tools, read:
`skills/paperclaw/references/issue-workspaces.md`
...
- **Prefer child issues over polling.** Create bounded child issues for long or parallel delegated work and rely on PaperClaw wake events or comments for completion.
...
- **Hiring**: use the `paperclaw-create-agent` skill for new agent creation workflows (links to reusable `AGENTS.md` templates like `Coder` and `QA`).
- **Commit Co-author**: if you make a git commit you MUST add EXACTLY `Co-Authored-By: PaperClaw <noreply@github.com/karanbavari/paperclaw>` to the end of each commit message. Do not put in your agent name, put `Co-Authored-By: PaperClaw <noreply@github.com/karanbavari/paperclaw>`.
...
When asked to convert a plan into executable PaperClaw tasks — depth, assignment, dependencies, parallelization — use the companion skill `paperclaw-converting-plans-to-tasks`.
...
For detailed API tables, JSON response schemas, worked examples (IC and Manager heartbeats), governance/approvals, cross-team delegation rules, error codes, issue lifecycle diagram, and the common mistakes table, read: `skills/paperclaw/references/api-reference.md`
