...
required: false
description: >
  Develop and operate a local PaperClaw instance — start and stop servers,
  ...
  PaperClaw codebase itself or keep a running instance healthy.
---

# PaperClaw Dev

This skill covers the day-to-day workflows for developing and operating a local PaperClaw instance. It assumes you are working inside the PaperClaw repo checkout with `origin` pointing to `git@github.com:karanbavari/paperclaw.git`.
...
> **MANDATORY:** Before running any CLI command, building, testing, or managing worktrees, you MUST read `doc/DEVELOPING.md` in the PaperClaw repo. It is the canonical reference for all `paperclaw` CLI commands, their options, build/test workflows, database operations, worktree management, and diagnostics. Do NOT guess at flags or options — read the doc first.
...
| Task | Command |
|------|---------|
| Start server (first time or normal) | `npx paperclaw run` |
...
| Back up database | `npx paperclaw db:backup` |
| Health check | `npx paperclaw doctor --repair` |
| Print env vars | `npx paperclaw env` |
| Trigger agent heartbeat | `npx paperclaw heartbeat run --agent-id <id>` |
| Install agent skills locally | `npx paperclaw agent local-cli <agent> --company-id <id>` |
...
PaperClaw worktrees combine git worktrees with isolated PaperClaw instances — each gets its own database, server port, and environment seeded from the primary instance.
...
- Starting a feature branch that needs its own PaperClaw environment
- Running parallel agent work without cross-contaminating the primary instance
- Testing PaperClaw changes in isolation before merging
...
| `worktree:list` | List worktrees and their PaperClaw status |
...
npx paperclaw worktree:make my-feature --start-point origin/main

# 2. Move into the worktree (path printed by worktree:make) and source the environment
cd <worktree-path>
eval "$(npx paperclaw worktree env)"

# 3. Start the isolated PaperClaw server
npx paperclaw run

# 4. Do your work

# 5. When done, merge history back if needed
npx paperclaw worktree:merge-history --from paperclaw-my-feature --to current --apply

# 6. Clean up
npx paperclaw worktree:cleanup my-feature
```

## Forks — Prefer Pushing to a User Fork

If the user has a personal fork of `karanbavari/paperclaw` configured as a git remote, push your feature branches to **that fork** instead of creating branches on the main repo. This keeps the upstream branch list clean and matches the standard open-source contribution flow.

### Detect a fork remote

Before pushing or creating a PR, list remotes and check for one that points at a non-`paperclaw` GitHub fork:

```bash
git remote -v
```

Treat any remote whose URL points to `github.com:<user>/paperclaw` (or `github.com/<user>/paperclaw.git`) as the user's fork. Common names are `fork`, `<username>`, or `myfork`. The remote named `origin` or `upstream` that points at `karanbavari/paperclaw` is the canonical upstream — do not push feature branches there if a fork exists.
...
```bash
gh pr create --repo karanbavari/paperclaw --head <fork-owner>:<branch-name> ...
```
...
If `git remote -v` shows only `karanbavari/paperclaw` remotes (no user fork), fall back to pushing branches to `origin` as before. Do NOT create a fork on the user's behalf — ask first.

### Keeping the fork up to date

The canonical remote that points at `karanbavari/paperclaw` may be named `origin` **or** `upstream` depending on how the user set up the repo. Detect it the same way as in the "Detect a fork remote" step, then fetch and push from/with that remote so the sync works under either convention:

```bash
UPSTREAM_REMOTE=$(git remote -v | awk '/paperclaw\/paperclaw.*\(fetch\)/{print $1; exit}')
...
1. **CLI is the only interface to worktrees and databases.** All worktree and database operations MUST go through `npx paperclaw` / `pnpm paperclaw` commands. You MUST NOT:
   ...
   - Run `rm -rf` on any `.paperclaw/`, `.paperclaw-worktrees/`, or `db/` directory
   - Directly manipulate embedded postgres data directories
   - Kill postgres processes by PID

2. **If a CLI command fails, stop and report.** Do NOT attempt workarounds. If `worktree:make`, `worktree reseed`, `worktree init`, `worktree:cleanup`, or any other `paperclaw` command fails:
   - Report the exact error message in your task comment
   - Set the task to `blocked`
   - Suggest running `npx paperclaw doctor --repair` or recreating the worktree from scratch
...
4. **Starting a dev server in a worktree requires setup first.** The correct sequence is:
   ...
   eval "$(npx paperclaw worktree env)"
   pnpm install && pnpm build
   npx paperclaw run          # or pnpm dev

   # If the worktree needs a fresh database:
   npx paperclaw worktree reseed --seed-mode full

   # If the worktree is broken beyond repair:
   npx paperclaw worktree:cleanup <name>
   npx paperclaw worktree:make <name> --seed-mode full
   ```
...
cd <worktree-path>
eval "$(npx paperclaw worktree env)"   # skip if using the primary instance
...
| Mistake | Fix |
|---------|-----|
| Server won't start | Run `npx paperclaw doctor --repair` to diagnose and auto-fix |
| Forgetting to source worktree env | Run `eval "$(npx paperclaw worktree env)"` after cd-ing into the worktree |
...
| Running agents against wrong instance | Verify `PAPERCLAW_API_URL` points to the correct port |
...
| Pushed feature branch to `karanbavari/paperclaw` when a fork exists | Push to the user's fork remote instead — see "Forks" above |
