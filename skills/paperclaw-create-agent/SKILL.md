...
description: >
  Create new agents in PaperClaw with governance-aware hiring. Use when you need
  to inspect adapter configuration options, compare existing agent configs,
  draft a new agent prompt/config, and submit a hire request.
---

# PaperClaw Create Agent Skill
...
```sh
curl -sS "$PAPERCLAW_API_URL/api/agents/me" \
  -H "Authorization: Bearer $PAPERCLAW_API_KEY"
```

### 2. Discover adapter configuration for this PaperClaw instance

```sh
curl -sS "$PAPERCLAW_API_URL/llms/agent-configuration.txt" \
  -H "Authorization: Bearer $PAPERCLAW_API_KEY"

# Then the specific adapter you plan to use, e.g. claude_local:
curl -sS "$PAPERCLAW_API_URL/llms/agent-configuration/claude_local.txt" \
  -H "Authorization: Bearer $PAPERCLAW_API_KEY"
```
...
```sh
curl -sS "$PAPERCLAW_API_URL/api/companies/$PAPERCLAW_COMPANY_ID/agent-configurations" \
  -H "Authorization: Bearer $PAPERCLAW_API_KEY"
```
...
`skills/paperclaw-create-agent/references/agent-instruction-templates.md`

Generic fallback for no-template hires:
`skills/paperclaw-create-agent/references/baseline-role-guide.md`
...
```sh
curl -sS "$PAPERCLAW_API_URL/llms/agent-icons.txt" \
  -H "Authorization: Bearer $PAPERCLAW_API_KEY"
```
...
- for coding or execution agents, include the PaperClaw execution contract: start actionable work in the same heartbeat; do not stop at a plan unless planning was requested; leave durable progress with a clear next action; use child issues for long or parallel delegated work instead of polling; mark blocked work with owner/action; respect budget, pause/cancel, approval gates, and company boundaries
...
`skills/paperclaw-create-agent/references/draft-review-checklist.md`
...
- when the board approves, you will be woken with `PAPERCLAW_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$PAPERCLAW_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $PAPERCLAW_API_KEY"

curl -sS -X POST "$PAPERCLAW_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $PAPERCLAW_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$PAPERCLAW_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $PAPERCLAW_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$PAPERCLAW_API_URL/api/approvals/$PAPERCLAW_APPROVAL_ID" \
  -H "Authorization: Bearer $PAPERCLAW_API_KEY"

curl -sS "$PAPERCLAW_API_URL/api/approvals/$PAPERCLAW_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $PAPERCLAW_API_KEY"
```
...
- Template index and how to apply a template: `skills/paperclaw-create-agent/references/agent-instruction-templates.md`
- Individual role templates: `skills/paperclaw-create-agent/references/agents/`
- Generic baseline role guide (no-template fallback): `skills/paperclaw-create-agent/references/baseline-role-guide.md`
- Pre-submit draft-review checklist: `skills/paperclaw-create-agent/references/draft-review-checklist.md`
- Endpoint payload shapes and full examples: `skills/paperclaw-create-agent/references/api-reference.md`
