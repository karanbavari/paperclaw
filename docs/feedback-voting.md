...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), PaperClaw saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
```bash
pnpm paperclaw feedback report
```
...
```bash
# Installed CLI
paperclaw feedback report

# Point to a different server or company
pnpm paperclaw feedback report --api-base http://127.0.0.1:3000 --company-id <company-id>

# Include raw payload dumps in the report
pnpm paperclaw feedback report --payloads
```
...
```bash
pnpm paperclaw feedback export
```
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json      # PaperClaw feedback envelope (one per trace)
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` keeps the PaperClaw envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.

```bash
# Custom server and output directory
pnpm paperclaw feedback export --api-base http://127.0.0.1:3000 --company-id <company-id> --out ./my-export
```
...
- Default endpoint: when no feedback export backend URL is configured, PaperClaw falls back to `https://telemetry.github.com/karanbavari/paperclaw`
