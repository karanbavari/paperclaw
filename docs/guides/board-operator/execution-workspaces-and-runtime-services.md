...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in PaperClaw.

PaperClaw now presents this as a workspace-command model:
...
- PaperClaw does not automatically start or stop these workspace services as part of issue execution.
- PaperClaw also does not automatically restart workspace services on server boot.
...
2. PaperClaw realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. PaperClaw persists execution-workspace metadata such as paths, refs, and provisioning settings.
