...
summary: Deploy PaperClaw to AWS using ECS Fargate, RDS Postgres, and EFS
---

Deploy PaperClaw to AWS with ECS Fargate (compute), RDS Postgres 17 (database), and EFS (persistent storage). This guide uses the AWS CLI and produces a single-task ECS service behind an ALB with HTTPS.
...
- The PaperClaw repo cloned locally
...
export PAPERCLAW_DOMAIN=paperclaw.example.com   # your domain
...
aws ecr create-repository \
  --repository-name paperclaw-server \
...
# Build
docker build -t paperclaw-server .

# Tag and push
docker tag paperclaw-server:latest \
  $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/paperclaw-server:latest
...
ALB_SG=$(aws ec2 create-security-group \
  --group-name paperclaw-alb \
  --description "PaperClaw ALB" \
  --vpc-id $VPC_ID \
...
ECS_SG=$(aws ec2 create-security-group \
  --group-name paperclaw-ecs \
  --description "PaperClaw ECS tasks" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
RDS_SG=$(aws ec2 create-security-group \
  --group-name paperclaw-rds \
  --description "PaperClaw RDS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
EFS_SG=$(aws ec2 create-security-group \
  --group-name paperclaw-efs \
  --description "PaperClaw EFS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
aws rds create-db-subnet-group \
  --db-subnet-group-name paperclaw-db-subnet \
  --db-subnet-group-description "PaperClaw RDS subnets" \
  --subnet-ids $SUBNET_1 $SUBNET_2

aws rds create-db-instance \
  --db-instance-identifier paperclaw-db \
  --db-instance-class db.t4g.micro \
  --engine postgres \
  --engine-version 17 \
  --master-username paperclaw \
...
DATABASE_URL="postgresql://paperclaw:${DB_PASSWORD}@${RDS_ENDPOINT}:5432/paperclaw"
```
...
EFS_ID=$(aws efs create-file-system \
  ...
  --encrypted \
  --tags Key=Name,Value=paperclaw-data \
...
aws secretsmanager create-secret \
  --name paperclaw/database-url \
  --secret-string "$DATABASE_URL"

aws secretsmanager create-secret \
  --name paperclaw/anthropic-api-key \
  --secret-string "YOUR_ANTHROPIC_KEY"

aws secretsmanager create-secret \
  --name paperclaw/better-auth-secret \
  --secret-string "$AUTH_SECRET"

aws secretsmanager create-secret \
  --name paperclaw/openai-api-key \
  --secret-string "YOUR_OPENAI_KEY"

aws secretsmanager create-secret \
  --name paperclaw/github-token \
...
# Task execution role
aws iam create-role \
  --role-name paperclaw-ecs-execution \
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      ...
      "Resource": "arn:aws:secretsmanager:'$AWS_REGION':'$AWS_ACCOUNT_ID':secret:paperclaw/*"
    }]
  }'

# Task role (application — add permissions as needed)
aws iam create-role \
  --role-name paperclaw-ecs-task \
  --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
...
```bash
aws ecs create-cluster --cluster-name paperclaw

aws logs create-log-group --log-group-name /ecs/paperclaw
```
...
sed -e "s|<ACCOUNT_ID>|$AWS_ACCOUNT_ID|g" \
    ...
    -e "s|<EFS_ID>|$EFS_ID|g" \
    -e "s|<DOMAIN>|$PAPERCLAW_DOMAIN|g" \
    docker/ecs-task-definition.json > /tmp/paperclaw-task-def.json

aws ecs register-task-definition \
  --cli-input-json file:///tmp/paperclaw-task-def.json
```
...
- Create a CNAME or ALIAS record for `$PAPERCLAW_DOMAIN` -> `$ALB_DNS`
...
aws ecs create-service \
  --cluster paperclaw \
  --service-name paperclaw-server \
  --task-definition paperclaw-server \
  --desired-count 1 \
  --launch-type FARGATE \
  ...
  --load-balancers '[{
    ...
    "containerName": "paperclaw-server",
    "containerPort": 3100
  }]'
```
...
aws ecs describe-services \
  --cluster paperclaw \
  --services paperclaw-server \
  --query 'services[0].{desired:desiredCount,running:runningCount,status:status}'

# Check task health
aws ecs list-tasks --cluster paperclaw --service-name paperclaw-server
TASK_ARN=$(aws ecs list-tasks --cluster paperclaw --service-name paperclaw-server --query 'taskArns[0]' --output text)
aws ecs describe-tasks --cluster paperclaw --tasks $TASK_ARN \
  --query 'tasks[0].{status:lastStatus,health:healthStatus}'

# Check logs
aws logs tail /ecs/paperclaw --since 10m --follow

# Hit the health endpoint
curl -sf https://$PAPERCLAW_DOMAIN/api/health
```
...
aws ecs list-task-definitions \
  --family-prefix paperclaw-server \
  --sort DESC \
...
aws ecs update-service \
  --cluster paperclaw \
  --service paperclaw-server \
  --task-definition paperclaw-server:<PREVIOUS_REVISION>
```
...
```bash
aws rds stop-db-instance --db-instance-identifier paperclaw-db
aws rds start-db-instance --db-instance-identifier paperclaw-db
```
...
aws ecs update-service --cluster paperclaw --service paperclaw-server --desired-count 0
aws ecs delete-service --cluster paperclaw --service paperclaw-server --force
aws ecs delete-cluster --cluster paperclaw
...
aws rds delete-db-instance \
  --db-instance-identifier paperclaw-db \
  --final-db-snapshot-identifier paperclaw-db-final
aws rds wait db-instance-deleted --db-instance-identifier paperclaw-db
aws rds delete-db-subnet-group --db-subnet-group-name paperclaw-db-subnet
...
for s in database-url anthropic-api-key better-auth-secret openai-api-key github-token; do
  aws secretsmanager delete-secret --secret-id paperclaw/$s --force-delete-without-recovery
done
...
# 7. ECR
aws ecr delete-repository --repository-name paperclaw-server --force

# 8. IAM roles
aws iam delete-role-policy --role-name paperclaw-ecs-execution --policy-name SecretsAccess
aws iam detach-role-policy --role-name paperclaw-ecs-execution \
  --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
aws iam delete-role --role-name paperclaw-ecs-execution
aws iam delete-role --role-name paperclaw-ecs-task

# 9. Log group
aws logs delete-log-group --log-group-name /ecs/paperclaw
```
