...
PaperClaw supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm paperclaw allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, PaperClaw emits a one-time claim URL at startup:
...
```sh
PAPERCLAW_DEPLOYMENT_MODE=authenticated PAPERCLAW_BIND=lan pnpm paperclaw run
```
