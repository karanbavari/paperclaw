# ClipHub: Marketplace for PaperClaw Team Configurations
...
> The "app store" for whole-company AI teams — pre-built PaperClaw configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for PaperClaw-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | PaperClaw company (agents, projects, governance) |
...
A complete PaperClaw company configuration:
...
Single-agent configurations designed to plug into a PaperClaw org:
...
Portable skill files that any PaperClaw agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with PaperClaw's skill loading system
...
interface Listing {
  ...
  paperclawVersionMin: string;     // Minimum PaperClaw version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target PaperClaw company
...
| `POST` | `/api/listings/:id/install` | Install to PaperClaw company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target PaperClaw company (or create new)
  → ClipHub API calls PaperClaw API to:
  ...
  → Redirect to PaperClaw dashboard with new team running
```
...
### 5.3 Creator: Export from PaperClaw → Publish

```
Running PaperClaw company → "Export as Blueprint" (CLI or UI)
  → PaperClaw exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (PaperClaw brand blue/purple)
...
| Install flow | One-click deploy to PaperClaw company |
...
{
  "targetCompanyId": "uuid",         // Existing PaperClaw company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `paperclaw-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as PaperClaw
- **Backend**: Node.js API (or extend PaperClaw server)
- **Database**: Postgres (can share PaperClaw's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with PaperClaw auth (or OAuth2)

### 9.2 Integration with PaperClaw

ClipHub can be:
- **Option A**: A separate app that calls PaperClaw's API to install blueprints
- **Option B**: A built-in section of the PaperClaw UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing PaperClaw UI and API.
...
- [ ] Install flow: blueprint → PaperClaw company
...
- [ ] "Export from PaperClaw" CLI command
