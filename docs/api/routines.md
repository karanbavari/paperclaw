...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-PaperClaw-Signature` + `X-PaperClaw-Timestamp` header pair matching the trigger's signing mode.
