...
Build a custom adapter to connect PaperClaw to any agent runtime.
...
|---|---|---|
| Source | Inside `paperclaw-fork` | Separate npm package |
| Distribution | Ships with PaperClaw | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying PaperClaw's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@kesarcloud/adapter-utils/server-utils`
2. Build environment with `buildPaperClawEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@kesarcloud/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildPaperClawEnv(agent)` | `@kesarcloud/adapter-utils/server-utils` | Inject `PAPERCLAW_*` env vars |
| `renderTemplate(tpl, data)` | `@kesarcloud/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@kesarcloud/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@kesarcloud/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in PaperClaw's source:
...
`format-event.ts` — pretty-prints stdout for `paperclaw run --watch` using `picocolors`.
...
With these flags set, the PaperClaw UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no PaperClaw source changes required.
...
Make PaperClaw skills discoverable to your agent runtime without writing to the agent's working directory:
