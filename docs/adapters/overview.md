...
summary: What adapters are and how they connect agents to PaperClaw
---

Adapters are the bridge between PaperClaw's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, PaperClaw:
...
|---------|---------|----------|-------------|
| Droid Local | `@henkey/droid-paperclaw-adapter` | `droid_local` | Runs Factory Droid locally |

## External Adapters

You can build and distribute adapters as standalone packages — no changes to PaperClaw's source code required. External adapters are loaded at startup via the plugin system.
...
my-adapter/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `paperclaw run --watch`
```
...
External adapters can ship a self-contained UI parser that tells the PaperClaw web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.
