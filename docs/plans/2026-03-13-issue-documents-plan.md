...
Add first-class **documents** to PaperClaw as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by PaperClaw workflow/docs.
...
Update the PaperClaw agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/paperclaw/SKILL.md`
